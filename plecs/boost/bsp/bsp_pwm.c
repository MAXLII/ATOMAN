...
/**
 * @file    bsp_pwm.c
 * @brief   PLECS boost PWM adapter module.
 * @details
 *          This file is part of the BUCK2 project.
 *
 *          Module responsibilities:
 *          - Convert boost compare commands into normalized PLECS duty outputs
 *          - Publish A/B complementary enable states to the simulation model
 *          - Provide the same PWM control surface used by the boost HAL
 *
 *          Design notes:
 *          - C11 compatible
...
static int32_t bsp_pwm_limit_cmp(int32_t cmp)
{
    ...
    {
        return BOOST_CTRL_CMP_MAX;
    }

    if (cmp < BOOST_CTRL_CMP_MIN)
    {
                                ...
                                uint8_t dn_en)
{
    ...
    duty = (float)cmp_limited / (float)BOOST_CTRL_CMP_MAX;
