...
- Section 自动注册：[SECTION_DESIGN.md](SECTION_DESIGN.md)
- Shell 调试变量：[SHELL_DESIGN.md](SHELL_DESIGN.md)
- Perf 性能测量：[PERF_DESIGN.md](PERF_DESIGN.md)
- Scope 波形捕获：[SCOPE_DESIGN.md](SCOPE_DESIGN.md)
- Trace 执行路径记录：[TRACE_DESIGN.md](TRACE_DESIGN.md)
- SFRA 扫频分析：[SFRA_DESIGN.md](SFRA_DESIGN.md)
...
```text
code/app/pfc/update.c
code/app/llc/update.c
```
...
并通过 `REG_COMM()` 注册升级命令，通过 `REG_TASK_MS()` 推进升级状态机。
...
5. 确认主循环持续调用 `run_task()`，否则 `update_task` 不会运行。
...
### 13.2 组件化升级代码恢复后的接入原则

如果恢复 `code/app/update/` 组件化升级代码，接入方式应保持以下边界：

1. 升级服务只依赖 `comm.c` 收发协议。
2. Flash 擦写不直接依赖 FAL 全局函数。
3. 平台侧提供 Flash 操作适配函数。
4. 平台侧把 FAL 实体函数挂到升级组件的函数指针表。
5. APP 起始地址、Bootloader 大小和 FAL 分区必须一致。

Flash 适配代码应放在平台或业务适配文件中，不放在升级服务核心文件中：

```c
#include "fal.h"
#include "update_port.h"

static int32_t update_flash_erase(uint32_t offset, uint32_t len)
{
    return fal_erase(FLASH_ZONE_APP_FW, offset, len);
}

static int32_t update_flash_write(uint32_t offset, const uint8_t *data, uint32_t len)
{
    return fal_write(FLASH_ZONE_APP_FW, offset, len, data);
}

static int32_t update_flash_read(uint32_t offset, uint8_t *data, uint32_t len)
{
    return fal_read(FLASH_ZONE_APP_FW, offset, len, data);
}
```
