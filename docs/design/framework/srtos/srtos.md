...
通用RTOS把任务资格、优先级选择、阻塞唤醒、同步通信、时间管理、独立栈和处理器上下文切换组合为一套并发模型。应用设计需要明确任务周期、截止期、最坏执行时间、阻塞关系、数据所有权、栈预算和资源失败策略；内核负责执行规则，但不会自动提供实时保证。

SRTOS只选择其中与静态周期任务和长流程让渡有关的部分。通用调度、任务状态、同步对象、响应时间和并发可靠性的基础原理分别见[实时任务与调度基础](../../../tutorial/realtime_task_scheduling.md)和[RTOS并发、内存与可靠性基础](../../../tutorial/rtos_concurrency_and_reliability.md)，本设计只保留与当前SRTOS结构直接有关的比较。

### 2.2 通用 RTOS 与 SRTOS 的设计对比
