...
FSM 负责配置发布和运行许可，内部进入运行回调只负责准备控制状态及执行硬件启动动作；INV、LLC、CLLC 和整数 PFC 还会在该阶段直接使能相应 PWM，Boost 则在快速控制路径满足条件后使能输出。浮点 PFC 的当前进入回调只准备控制器。

完整进入过程可包含：

1. FSM 发布完整 building 配置并设置 `run_allowed`；
2. 重置或准备控制器历史状态；
3. 根据拓扑在生命周期路径或后续控制路径启用功率输出；
4. 从后续控制周期开始产生有效调制。

停止时先由 HAL 关闭 PWM 或结束功率传输，再由 FSM 撤销并发布 `run_allowed`。硬故障路径立即关闭 PWM，并向 FSM 投递 stop；FSM 在后续调度中完成状态与配置收敛。

无论采用哪种停止时序，关闭回调都必须幂等：重复调用、尚未启用时调用和故障中调用都应保持功率级关闭。平台还应验证从 stop 到实际无脉冲的最坏延迟是否满足功率级要求。

部分拓扑允许平台替换进入、退出回调。替换者必须保留控制器准备和 PWM 安全关闭等硬件生命周期不变量；配置发布与运行许可仍由 FSM 统一管理。
