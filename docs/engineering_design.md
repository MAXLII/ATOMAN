...
| 模块 | `*_core.c/.h` | `*.c/.h` | `*_service.c/.h` |
|---|---|---|---|
| Shell | 变量、命令、表达式解析和显式列表接口 | Section 注册发现、链表构建和兼容 API | 二进制读写与波形上报 |
| Scope | 多通道采样、触发和环形缓冲 | Section 注册描述符、链表和兼容 API | 配置、状态和采样数据传输 |
| Trace | 运行轨迹 FIFO 和显式时间源 | 兼容 API | 轨迹控制与分批上报 |
| Perf | 计时、负载统计和显式列表接口 | Section 注册、链表构建和兼容 API | 负载与性能记录查询 |
| SFRA | 扫频状态机、采样和响应计算 | Section 注册描述符、链表和兼容 API | 扫频配置、结果缓存、控制和查询 |

`*_core.c/.h` 只依赖 C11 标准库及所需的纯软件算法模块，通过调用方传入的对象、链表访问接口、时间源或输出接口运行。`*.c/.h` 保留现有工程使用的头文件、宏和函数名，负责 Section 扫描、注册对象和链表适配。`*_service.c/.h` 处理 FRAME 协议、Shell 服务命令和通信侧运行状态。业务模块继续包含对应的 `*.h`。
