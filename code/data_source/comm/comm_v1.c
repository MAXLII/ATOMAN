...
static uint8_t comm_v1_is_route_target(uint8_t link_id, uint8_t dst_addr)
{
    for (section_item_t *p_item = p_comm_route_first; p_item != NULL; p_item = p_item->p_next)
    {
        ...
        if ((r->src_link_id == link_id) && (r->dst_addr == dst_addr))
        {
            return 1u;
        }
    }
...
static int8_t comm_v1_decode_data(uint8_t codec, uint16_t input_len,
                                  const uint8_t *p_input, uint16_t *p_output_len,
                                  uint8_t *p_output)
{
...
void comm_v1_run(uint8_t data, DEC_MY_PRINTF, void *p_context)
{
    ...
    case COMM_V1_STA_MSG_L0:
    {
        ...
        const uint8_t is_duplicate = (uint8_t)((ctx->last_seq_valid == 1u) &&
                                               (seq == ctx->last_seq));
        const uint8_t is_local_static = (uint8_t)((dst == 0u) || (dst == ctx->src));
        ...
        ctx->is_route = (uint8_t)((is_local_static == 0u) && (is_route_target == 1u));
    ...
    case COMM_V1_STA_MSG_L3:
    {
        ...
        msg_d_dst = (uint8_t)(((ctx->msg >> 7u) & 0x01u) |
                              (((ctx->msg >> 8u) & 0x03u) << 1u));
        ctx->pack.d_dst = msg_d_dst;
        ctx->pack.src = (uint8_t)((ctx->msg >> 10u) & 0x0Fu);
        ctx->pack.d_src = (uint8_t)(((ctx->msg >> 14u) & 0x03u) |
                                    (((ctx->msg >> 16u) & 0x01u) << 2u));
        ctx->pack.cmd_set = (uint8_t)((ctx->msg >> 17u) & 0x0Fu);
        ctx->pack.cmd_word = (uint8_t)(((ctx->msg >> 21u) & 0x07u) |
                                       (((ctx->msg >> 24u) & 0x07u) << 3u));
        ctx->pack.is_ack = (uint8_t)((ctx->msg >> 31u) & 0x01u);

        /* 编码合法性：纯命令帧必须 RAW，保留 CODEC 必须拒绝。 */
        if (((msg_cmd == 1u) && (msg_codec != COMM_V1_CODEC_RAW)) ||
    ...
    case COMM_V1_STA_SUM:
    {
        ...
        if (msg_codec != COMM_V1_CODEC_RAW)
        {
            ...
            int8_t decode_result = comm_v1_decode_data(msg_codec,
                                                       ctx->index,
                                                       ctx->p_data_buffer,
                                                       &output_len,
                                                       ctx->p_candidate_buffer);
            if (decode_result != 1)
            {
...
void comm_v1_run_buffer(const uint8_t *p_data, uint32_t length, DEC_MY_PRINTF, void *p_context)
{
    ...
    while (offset < length)
    {
        ...
        if ((p_ctx->status == COMM_V1_STA_DATA) && /* A validated payload is being received. */
...
void comm_v1_send(void *p_frame, DEC_MY_PRINTF)
{
    ...
    else
    {
            ...
            (comm_v1_is_codec_select(p_pack) == 0u))
        {
            ...
            codec_result = codec_dict_encode(raw_len, p_pack->p_data,
                                             &candidate_len, tx->candidate, limit_len);
            if ((codec_result == 1) && (candidate_len < best_len))
            {
            ...
            if (raw_len > COMM_V1_MIN_SAVED_RAW_SMALL)
            {
                candidate_len = COMM_V1_MAX_DATA;
                limit_len = (uint16_t)((best_len < (raw_len - min_saved + 1u)) ? best_len : (raw_len - min_saved + 1u));
                codec_result = codec_rle_encode(raw_len, p_pack->p_data,
                                                &candidate_len, tx->candidate, limit_len);
                if ((codec_result == 1) && (candidate_len < best_len))
                {
                    (void)memcpy(&tx_buffer[COMM_V1_FRAME_PREFIX], tx->candidate, candidate_len);
            ...
            if (raw_len >= 64u)
            {
                candidate_len = COMM_V1_MAX_DATA;
                limit_len = (uint16_t)((best_len < (raw_len - min_saved + 1u)) ? best_len : (raw_len - min_saved + 1u));
                codec_result = codec_lzss_encode(raw_len, p_pack->p_data,
                                                 &candidate_len, tx->candidate, limit_len);
                if ((codec_result == 1) && (candidate_len < best_len))
                {
                    (void)memcpy(&tx_buffer[COMM_V1_FRAME_PREFIX], tx->candidate, candidate_len);
            ...
            /* 候选 4：ZERO 数零法（所有长度，小值密集数据受益）。 */
            {
                candidate_len = COMM_V1_MAX_DATA;
                limit_len = (uint16_t)((best_len < (raw_len - min_saved + 1u)) ? best_len : (raw_len - min_saved + 1u));
                codec_result = codec_zero_encode(raw_len, p_pack->p_data,
                                                 &candidate_len, tx->candidate, limit_len);
                if ((codec_result == 1) && (candidate_len < best_len))
                {
                    (void)memcpy(&tx_buffer[COMM_V1_FRAME_PREFIX], tx->candidate, candidate_len);
    ...
    msg = (uint32_t)p_pack->seq |
          ((uint32_t)p_pack->dst << 3u) |
          ((uint32_t)p_pack->d_dst << 7u) |
          ((uint32_t)p_pack->src << 10u) |
          ((uint32_t)p_pack->d_src << 14u) |
          ((uint32_t)p_pack->cmd_set << 17u) |
          ((uint32_t)p_pack->cmd_word << 21u) |
          ((uint32_t)best_codec << 27u) |
          ((uint32_t)cmd << 30u) |
          ((uint32_t)ack << 31u);
