...
static const demo_storage_params_t defaults = { /* 默认值属于数据源，不属于数据池。 */
                                               .input_mv     = 1000u,
                                               .gain_milli   = 1000u,
                                               .threshold_mv = 1500u};
...
static uint8_t transfer(uint8_t op,
                        uint8_t zone,
                        uint32_t offset,
                        uint32_t length,
                        uint8_t *p_buffer)
...
static void decide(void)
{
    ...
    demo_storage_record_encode(data_page,
                               commit_page,
                               p_devices[d]->program_page_size,
                               (request.command == DEMO_STORAGE_SAVE) ? 1u : 2u,
                               next_sequence,
                               &request.params);
    phase = STORAGE_ERASE;
}
...
void demo_storage_service_process(void)
{
         ...
         || (phase == STORAGE_SCAN_COMMIT))
    {
        ...
        if (phase == STORAGE_SCAN_DATA)
        {
            ...
            if (    (io_result == FAL_RESULT_SUCCESS)
                 && (demo_storage_record_data_valid(data_page, kind) == 1u))
            {
                phase = STORAGE_SCAN_COMMIT;
            }
            else
        ...
        else
        {
            ...
            if (    (io_result == FAL_RESULT_SUCCESS)
                 && (demo_storage_record_committed(data_page, commit_page) == 1u))
            {
    ...
    if (phase == STORAGE_FINISH)
    {
        ...
        if (demo_storage_exchange_finish(request.id, outcome, (restore == 1u) ? &recovered[d] : NULL, views) == 1u)
        {
            phase   = STORAGE_IDLE;
            booting = 0u;
        }
    ...
    else if (phase == STORAGE_VERIFY)
    {
        ...
        if (    (io_result == FAL_RESULT_SUCCESS)
             && (memcmp(data_page, verify_page, page) != 0))
        {
            outcome = -21;
            phase   = STORAGE_FINISH;
            return;
    ...
    else if (phase == STORAGE_VERIFY_COMMIT)
    {
        ...
        if (    (io_result == FAL_RESULT_SUCCESS)
             && (memcmp(commit_page, verify_page, page) != 0))
        {
            outcome = -21;
        }
        ...
        if (    (io_result == FAL_RESULT_SUCCESS)
             && (outcome == 0)
             && (request.command == DEMO_STORAGE_SAVE))
        {
            views[d].valid    = 1u;
    ...
    if (    (phase == STORAGE_FINISH)
         && (io_result == FAL_RESULT_SUCCESS)
         && (outcome == 0)
         && (request.command == DEMO_STORAGE_LOG))
    {
