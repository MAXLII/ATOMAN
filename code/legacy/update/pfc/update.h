...
#define CMD_WROD_UPDATE_FW 0x0A
...
typedef enum
{
    ...
    UPDATE_FSM_ERASE_EXTER_FLASH,
    UPDATE_FSM_READY_PFC_UPDATE,
    UPDATE_FSM_RECEIVE_FW,
    UPDATE_FSM_ERASE_INTER_FLASH,
    UPDATE_FSM_READ_FLASH,
    UPDATE_FSM_OVERWRITE,
...
typedef struct
{
    ...
    uint8_t packet_data[256]; // 小包数据 数据不够时用0xFF来凑齐
...
typedef struct
{
    ...
    uint8_t module_id;         // 模块ID
    uint16_t data_length;      // 数据长度 实际有效的数据长度
    uint8_t packet_data[1024]; // 小包数据 数据不够时用0xFF来凑齐
    uint16_t packet_crc;       // 本数据包的CRC16
} update_fw_pack_1024_t;
