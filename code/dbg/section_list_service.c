 ...
 *          - Build and validate the directory of debug-visible runtime lists
...
{
    return (uint32_t)p_src[0] | ((uint32_t)p_src[1] << 8u) | ((uint32_t)p_src[2] << 16u) | ((uint32_t)p_src[3] << 24u);
}
...
static uint8_t section_list_service_name_len(const char *p_name)
{
    ...
    while (    (length < SECTION_LIST_NAME_LEN_MAX)
            && (p_name[length] != '\0'))
    {
        length++;
    }
...
static uint8_t section_list_service_registration_is_valid(const section_list_registration_t *p_registration)
{
    ...
    if (    (name_length == 0u)
         || (p_registration->p_name[name_length] != '\0'))
    {
        return 0u;
    }
    return 1u;
...
static const section_list_registration_t *section_list_service_registration_get(uint16_t directory_index)
{
    uint16_t current_index         = 0u;
    const reg_section_t *p_section = NULL;

    for (p_section = (const reg_section_t *)&SECTION_START; p_section < (const reg_section_t *)&SECTION_STOP;
         p_section++)
    {
...
static void section_list_directory_act(void *p_frame, DEC_MY_PRINTF)
{
    ...
    if (    (p_pack->p_data == NULL)
         || (p_pack->len < SECTION_LIST_DIRECTORY_REQUEST_SIZE))
    {
    ...
    {
        const section_list_registration_t *p_registration = section_list_service_registration_get(requested_index);
    ...
    section_list_service_reply(p_pack, my_printf, CMD_WORD_SECTION_LIST_DIRECTORY, payload, payload_length);
}
...
static void section_list_node_act(void *p_frame, DEC_MY_PRINTF)
{
    ...
    if (    (p_pack->p_data == NULL)
         || (p_pack->len < SECTION_LIST_NODE_REQUEST_SIZE))
    {
        status = (uint8_t)SECTION_LIST_STATUS_INVALID_REQUEST;
    }
    else
    ...
    section_list_service_reply(p_pack, my_printf, CMD_WORD_SECTION_LIST_NODE, payload, payload_length);
}
