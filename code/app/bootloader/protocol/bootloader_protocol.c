                                       ...
                                       uint16_t *p_ack_length)
{
    ...
    p_ack[offsetof(bootloader_protocol_info_ack_t, allow_update)] =
        (result == BOOTLOADER_RESULT_SUCCESS_E) ? (uint8_t)BOOTLOADER_PROTOCOL_UPDATE_ACK_ALLOW_E
                                        ...
                                        uint16_t *p_ack_length)
{
    ...
    p_ack[offsetof(bootloader_protocol_ready_ack_t, ready)] = bootloader_is_download_ready(p_protocol->p_bootloader);
                                       ...
                                       uint16_t *p_ack_length)
{
    ...
    if (
        (p_payload[offsetof(bootloader_protocol_data_request_t, module_id)] != p_protocol->p_bootloader->config.expected_module_id)
     || (data_length == 0u)
    ...
    result = bootloader_packet_submit(p_protocol->p_bootloader,
                                      ...
                                      data_length);
    p_ack[offsetof(bootloader_protocol_data_ack_payload_t, data_is_ok)] =
        (result == BOOTLOADER_RESULT_SUCCESS_E) ? (uint8_t)BOOTLOADER_PROTOCOL_DATA_VALID_E
                                                : (uint8_t)BOOTLOADER_PROTOCOL_DATA_INVALID_E;
    *p_ack_length = BOOTLOADER_PROTOCOL_DATA_ACK_LENGTH;
    return result;
}
                                      ...
                                      uint16_t *p_ack_length)
{
    ...
    result = bootloader_upgrade_end(p_protocol->p_bootloader,
                                    read_u16_le(&p_payload[offsetof(bootloader_protocol_end_request_t, fw_crc)]));
    p_ack[offsetof(bootloader_protocol_end_ack_payload_t, success_flg)] = (result == BOOTLOADER_RESULT_SUCCESS_E)
...
{
    if (    (p_protocol_active != NULL)
         && (p_protocol_active->p_bootloader != NULL))
    {
