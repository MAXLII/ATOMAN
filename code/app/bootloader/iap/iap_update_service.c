...
static volatile bootloader_update_request_t boot_request IAP_UPDATE_RETAINED; /* Shared no-init request at the same address in both images. */
...
__attribute__((weak)) iap_update_prepare_result_t iap_update_prepare(const iap_update_info_t *p_info)
{
    (void)p_info;
...
static void update_info_act(void *p_frame, DEC_MY_PRINTF)
{
         ...
         && (transfer_pending == 0u))
    {
        ...
        prepare_ready                = 0u;
        reset_called                 = 0u;
        update_info_ack.allow_update = (uint8_t)BOOTLOADER_PROTOCOL_UPDATE_ACK_ALLOW_E;
    }
...
{
    iap_update_prepare_result_t prepare_result = IAP_UPDATE_PREPARE_FAILED_E; /* Result of the current preparation step. */
