...
typedef struct
{
    bootloader_result_t (*p_zone_info_get)(bootloader_flash_zone_t zone, bootloader_flash_zone_info_t *p_info);     /**< Query region geometry. */
    bootloader_result_t (*p_read)(bootloader_flash_zone_t zone, uint32_t offset, uint32_t length, uint8_t *p_data); /**< Submit a logical read. */
    bootloader_result_t (*p_write)(bootloader_flash_zone_t zone, uint32_t offset, uint32_t length,
                                   const uint8_t *p_data); /**< Submit a logical program. */
    bootloader_result_t (*p_erase)(bootloader_flash_zone_t zone, uint32_t offset, uint32_t length); /**< Submit a logical erase. */
...
typedef struct
{
    ...
    bootloader_result_t (*p_upgrade_info_get)(void *p_context, bootloader_upgrade_info_t *p_info); /**< Read an optional retained IAP request. */
    bootloader_result_t (*p_boot_reason_clear)(void *p_context); /**< Clear an accepted upgrade request. */
    bootloader_result_t (*p_image_header_is_valid)(void *p_context, const uint8_t *p_header, uint32_t header_length,
                                                   uint32_t image_size, uint8_t *p_valid); /**< Validate platform image entry/stack form. */
...
typedef struct
{
    ...
    uint16_t (*p_crc16_update)(const uint8_t *p_data, uint32_t length, uint16_t crc); /**< Continue the existing FRAME CRC16. */
} bootloader_config_t;
