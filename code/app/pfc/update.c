...
static uint8_t llc_update_ready;
static FLASH_ZONE_E write_target_zone;
static update_fw_pack_1024_t update_fw_pack = {0};
...
void update_task(void)
{
    ...
    case UPDATE_FSM_IDLE:
        if (fal_busy(FLASH_ZONE_UPDATE_INFO) == 0)
        {
            ...
            {
                write_target_zone = FLASH_ZONE_APP_FW; // 直接往APP区域写代码
                host_addr = PFC_ADDR;
    ...
    case UPDATE_FSM_ERASE_EXTER_FLASH:
        ...
        fsm_sta = UPDATE_FSM_READY_PFC_UPDATE;
        break;
    case UPDATE_FSM_READY_PFC_UPDATE:
    ...
    case UPDATE_FSM_RECEIVE_FW:
        ...
        {
            if (update_fw_pack.offset == 0)
            {
                update_fw_pack.packet_data[0] = 0xFF;
                update_fw_pack.packet_data[1] = 0xFF;
                update_fw_pack.packet_data[2] = 0xFF;
                update_fw_pack.packet_data[3] = 0xFF;
            }
            write_flash_flg = 0;
                 ...
                 (fal_busy(write_target_zone) == 0))
        {
            ...
            if (update_fw_pack.offset == 0)
            {
                update_fw_pack.packet_data[0] = 0x00;
                update_fw_pack.packet_data[1] = 0x80;
                update_fw_pack.packet_data[2] = 0x00;
                update_fw_pack.packet_data[3] = 0x20;
            }
        ...
        {
            fal_erase(FLASH_ZONE_APP_FW, 0, 4); // 销毁stack区域
...
static uint8_t update_info_act_cnt = 0;

REG_SHELL_VAR(update_info_act_cnt, update_info_act_cnt, SHELL_UINT8, 255, 0, NULL, SHELL_STA_NULL)
...
static void update_info_act(section_packform_t *p_pack, DEC_MY_PRINTF)
{
    ...
    if ((p_update_fw_info->module_id != PFC_ADDR) &&
...
static void update_ready_act(section_packform_t *p_pack, DEC_MY_PRINTF)
{
    ...
    packform.d_src = p_pack->d_dst;
    packform.is_ack = 1;
    packform.len = sizeof(llc_update_ready);
    packform.p_data = (uint8_t *)&llc_update_ready;
    comm_send_data(&packform, my_printf);
}
...
static void update_fw_pack_act(section_packform_t *p_pack, DEC_MY_PRINTF)
{
        ...
        (fal_busy(write_target_zone) == 0))
    {
        ...
        if ((p_update_fw_pack->module_id == host_addr) &&
            (p_update_fw_pack->packet_crc == section_crc16((uint8_t *)p_update_fw_pack,
    ...
    else if ((sizeof(update_fw_pack_1024_t) == p_pack->len) &&
             (fal_busy(write_target_zone) == 0))
    {
        update_fw_pack_1024_t *p_update_fw_pack = (update_fw_pack_1024_t *)p_pack->p_data;
        if ((p_update_fw_pack->module_id == host_addr) &&
            (p_update_fw_pack->packet_crc == section_crc16((uint8_t *)p_update_fw_pack,
                                                           sizeof(update_fw_pack_1024_t) -
                                                               sizeof(update_fw_pack.packet_crc))))
        {
            memcpy((uint8_t *)&update_fw_pack, p_pack->p_data, sizeof(update_fw_pack_1024_t));
            write_flash_flg = 1;
        }
        else
        {
