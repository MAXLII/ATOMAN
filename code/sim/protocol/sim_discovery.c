...
static void discovery_reply(const struct sockaddr_in *p_remote, const sim_discovery_reg_t *p_reg)
{
    ...
    int count = snprintf(response, sizeof(response),
        ...
        p_reg->p_name, (unsigned int)p_reg->node_addr,
        (unsigned int)p_reg->tcp_port, (unsigned int)p_reg->node_addr);
...
{
    if ((ready == 0u) || (p_registered == NULL)) return;
    if (discovery == INVALID_SOCKET)
    {
        if ((uint32_t)(now - discovery_retry_ms) < SIM_RETRY_MS) return;
    ...
    for (uint32_t i = 0u; i < 8u; ++i)
    {
        ...
        if ((received == 18) && (memcmp(request, "FRAME_DISCOVER_V1", 17u) == 0) && (request[17] == '\0'))
        {
            received = 17; /* Accept both raw and NUL-terminated discovery probes. */
        }
        if ((received == 17) && (memcmp(request, "FRAME_DISCOVER_V1", 17u) == 0))
        {
...
{
    if (discovery != INVALID_SOCKET) (void)closesocket(discovery);
