...
static void connect_poll(uint32_t index, uint32_t now)
{
    ...
    if (p_link->socket != INVALID_SOCKET) return;
    if (client == 0u)
    {
        if (p_link->listener == INVALID_SOCKET)
        {
            if ((uint32_t)(now - p_link->retry_ms) < SIM_RETRY_MS) return;
            p_link->retry_ms = now;
        ...
        if (p_link->listener != INVALID_SOCKET)
        {
            ...
            {
                if (configure_socket(p_link->socket) == 1u) connected(index);
                else disconnect_link(index);
            }
        }
        return;
...
static void transfer_poll(uint32_t index)
{
    ...
    if ((p_link->socket == INVALID_SOCKET) || (p_link->connecting == 1u)) return;
    if (count > SIM_RX_CAPACITY - tail) count = SIM_RX_CAPACITY - tail;
    if (count > SIM_IO_BUDGET) count = SIM_IO_BUDGET;
    if (count > 0u)
    {
        result = recv(p_link->socket, (char *)&p_link->rx[tail], (int)count, 0);
        if (result > 0) p_link->rx_count += (uint32_t)result;
    ...
    if (count > SIM_TX_CAPACITY - p_link->tx_head) count = SIM_TX_CAPACITY - p_link->tx_head;
    if (count > SIM_IO_BUDGET) count = SIM_IO_BUDGET;
    if (count > 0u)
    {
        ...
        else if (WSAGetLastError() != WSAEWOULDBLOCK) disconnect_link(index);
    }
}
...
static void sim_tcp_init(void)
{
    ...
    for (section_item_t *p_item = p_registered; p_item != NULL; p_item = p_item->p_next)
    {
        ...
        *p_reg->p_handle = NULL;
        ...
        if ((p_reg->p_name == NULL) || (p_reg->p_name[0] == '\0') ||
            (p_reg->p_ip == NULL) || (p_reg->port == 0u) ||
            ((p_reg->role != SIM_TCP_SERVER) && (p_reg->role != SIM_TCP_CLIENT)) ||
...
void sim_tcp_tx(struct sim_tcp *p_tcp, const char *p_data, int len)
{
    ...
    if ((p_link == NULL) || (p_data == NULL) || (len <= 0)) return;
    if ((ready == 0u) || (p_link->socket == INVALID_SOCKET) ||
        (p_link->connecting == 1u) || ((uint32_t)len > SIM_TX_CAPACITY - p_link->tx_count))
    {
        if (p_link->p_reg->status.tx_dropped != UINT32_MAX) ++p_link->p_reg->status.tx_dropped;
        return;
    }
...
uint8_t sim_tcp_rx_get_byte(struct sim_tcp *p_tcp, uint8_t *p_data)
{
    ...
    if ((p_link == NULL) || (p_data == NULL) || (p_link->rx_count == 0u)) return 0u;
...
void sim_tcp_vprintf(struct sim_tcp *p_tcp, const char *p_format, va_list args)
{
    ...
    if ((length > 0) && ((size_t)length < sizeof(text))) sim_tcp_tx(p_tcp, text, length);
}
