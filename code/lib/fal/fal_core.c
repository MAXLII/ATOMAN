                                    ...
                                    uint16_t zone_index)
{
    ...
    for (compare_device_index = 0u; compare_device_index <= device_index; compare_device_index++)
    {
        const fal_device_cfg_t *p_device = &p_cfg->p_devices[compare_device_index];
        const uint16_t compare_zone_count = (compare_device_index == device_index) ? zone_index : p_device->zone_count;
        uint16_t compare_zone_index = 0u;

        for (compare_zone_index = 0u; compare_zone_index < compare_zone_count; compare_zone_index++)
        {
...
static fal_result_t operation_issue(fal_t *p_fal, const fal_device_cfg_t *p_device)
{
    ...
    if (p_fal->operation == FAL_OPERATION_READ)
    {
        ...
        result = p_device->ops.p_read(p_device->ops.p_context, p_fal->physical_address, chunk, p_fal->p_read_data);
    }
    else if (p_fal->operation == FAL_OPERATION_WRITE)
    {
        chunk  = write_chunk_get(p_fal, p_device);
        result = p_device->ops.p_program(p_device->ops.p_context, p_fal->physical_address, chunk, p_fal->p_write_data);
    }
    else if (p_fal->operation == FAL_OPERATION_ERASE)
    {
        chunk  = p_device->erase_block_size;
        result = p_device->ops.p_erase(p_device->ops.p_context, p_fal->physical_address, chunk);
    }
    else
    {
        return FAL_RESULT_CONFIG_ERROR;
...
fal_result_t fal_init(fal_t *p_fal, const fal_cfg_t *p_cfg)
{
    ...
    for (index = 0u; index < p_cfg->device_count; index++)
    {
        ...
        for (zone_index = 0u; zone_index < p_cfg->p_devices[index].zone_count; zone_index++)
        {
                      ...
                      uint8_t *p_data)
{
    ...
    result =
        request_validate(p_fal, zone_id, offset, length, FAL_ZONE_PERMISSION_READ, &p_zone, &p_device, &device_offset);
    ...
    request_begin(p_fal, p_device, FAL_OPERATION_READ, FAL_STATE_READ, device_offset + offset, length);
                       ...
                       const uint8_t *p_data)
{
    ...
    result =
        request_validate(p_fal, zone_id, offset, length, FAL_ZONE_PERMISSION_WRITE, &p_zone, &p_device, &device_offset);
    ...
    request_begin(p_fal, p_device, FAL_OPERATION_WRITE, FAL_STATE_WRITE, device_offset + offset, length);
                       ...
                       uint32_t length)
{
    ...
    result =
        request_validate(p_fal, zone_id, offset, length, FAL_ZONE_PERMISSION_ERASE, &p_zone, &p_device, &device_offset);
