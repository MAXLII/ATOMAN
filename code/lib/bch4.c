...
static uint32_t bit_get(const uint8_t *p_data, const uint8_t *p_ecc, uint32_t bit)
{
    if (bit < 52u)
    {
        return ((uint32_t)p_ecc[bit / 8u] >> (bit % 8u)) & 1u;
    }
    bit -= 52u;
...
static uint8_t syndromes(const uint8_t *p_data, const uint8_t *p_ecc, uint16_t *p_s)
{
    ...
    for (uint32_t bit = 0u; bit < BCH4_BITS; bit++)
    {
        for (uint32_t k = 0u; k < 4u; k++)
        {
            ...
            if (bit_get(p_data, p_ecc, bit) == 1u)
            {
                p_s[2u * k + 1u] ^= powers[k];
            }
...
void bch4_encode(const uint8_t *p_data, uint8_t *p_ecc)
{
    ...
    for (uint32_t bit = BCH4_BITS; bit > 0u; bit--)
    {
        ...
        if ((remainder & (UINT64_C(1) << 52u)) != 0u)
        {
            remainder ^= BCH4_GENERATOR;
        }
    }

    for (uint32_t i = 0u; i < BCH4_ECC_SIZE; i++)
    {
        p_ecc[i] = (uint8_t)(remainder >> (8u * i));
    }
...
int32_t bch4_correct(uint8_t *p_data, uint8_t *p_ecc)
{
    ...
    if (syndromes(p_data, p_ecc, s) == 0u)
    {
        return 0;
    }
    ...
    for (uint32_t n = 0u; n < 8u; n++)
    {
        ...
        {
            discrepancy ^= multiply(locator[j], s[n + 1u - j]);
        }
        ...
        else
        {
            ...
            for (uint32_t j = 0u; j + shift < 9u; j++)
            {
                locator[j + shift] ^= multiply(ratio, previous[j]);
            }
    ...
    for (uint32_t bit = 0u; bit < BCH4_BITS; bit++)
    {
        ...
        if (value == 0u)
        {
            if (bit < 52u)
            {
                p_ecc[bit / 8u] ^= (uint8_t)(1u << (bit % 8u));
            }
            else
            {
