 ...
 */
static void FUNC_RAM calculate_average_balance(svpwm_3level_t *p_svpwm,
                                               const float *p_phase,
                                               const float *p_current,
                                               float scale)
{
    ...
    svpwm_3level_phase_output_t *phase_output[3] = {&p_svpwm->output.phase_a,
                                                    &p_svpwm->output.phase_b,
                                                    &p_svpwm->output.phase_c};
    ...
    slope_floor = fmaxf(p_cfg->midpoint_current_min, p_cfg->midpoint_slope_floor_ratio * p_inter->current_filtered)
                / (0.5f * (vp + vn));
...
static inline void limit_reference(const svpwm_3level_input_t *p_input, float scale, float *p_phase)
{
    ...
    inv_clarke(p_input->v_alpha / magnitude, p_input->v_beta / magnitude, &p_phase[0], &p_phase[1], &p_phase[2]);
    span = fmaxf(p_phase[0], fmaxf(p_phase[1], p_phase[2])) - fminf(p_phase[0], fminf(p_phase[1], p_phase[2]));
