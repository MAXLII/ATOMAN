                         ...
                         uint16_t limit_len)
{
    ...
    *p_output_len = 0u;
    ...
    for (index = 0u; index < input_len; ++index)
    {
        ...
        {
            const uint8_t nibble = (nibble_index == 0u)
                                       ? (uint8_t)(p_input[index] >> 4u)
...

    {
        ...
        if ((out_len == 0u) || (out_len > capacity) || (out_len >= limit_len))
        {
            return codec_zero_fail(p_output_len);
        }
                         ...
                         uint16_t limit_len)
{
    ...
    *p_output_len = 0u;
    ...
    while (bit_index < bit_total)
    {
        ...
        if ((offset_count == 0u) || (offset_count > (CODEC_ZERO_SEGMENT_MAX + 1u)))
        {
            return codec_zero_fail(p_output_len);
        }
