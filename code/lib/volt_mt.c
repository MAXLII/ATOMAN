...
void volt_mt_upd(volt_mt_t *mt, uint32_t curr_time)
{
    ...
    if (    voltage <= mt->cfg.min_inst
         || voltage >= mt->cfg.max_inst)
    {
    ...
    case ST_INVALID:
        ...
        if (    voltage > mt->cfg.min_vld
             && voltage < mt->cfg.max_vld)
        {
    ...
    case ST_VALID:
        ...
        if (    voltage <= mt->cfg.min_exit
             || voltage >= mt->cfg.max_exit)
        {
    ...
    case ST_EXITING:
        ...
        else if (    voltage <= mt->cfg.min_exit
                  || voltage >= mt->cfg.max_exit)
        {
            // 检查是否达到退出延时
