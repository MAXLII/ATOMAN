...
static FLASH_PORT_RESULT_E result_map(BSP_FLASH_RESULT_E result)
{
    switch (result)
    {
    case BSP_FLASH_SUCCESS:
        return FLASH_PORT_SUCCESS;
    case BSP_FLASH_INVALID_ARGUMENT:
        return FLASH_PORT_INVALID_ARGUMENT;
    default:
        return FLASH_PORT_IO_ERROR;
    }
}
...
FLASH_PORT_STATE_E flash_port_state_get(uint8_t device)
{
    ...
    {
    case BSP_FLASH_READY:
        return FLASH_PORT_READY;
    case BSP_FLASH_BUSY:
        return FLASH_PORT_BUSY;
    default:
        return FLASH_PORT_ERROR;
    }
}
FLASH_PORT_RESULT_E flash_port_read(uint8_t device,
                                    uint32_t address,
                                    uint32_t length,
                                    uint8_t *p_data)
{
...
FLASH_PORT_RESULT_E flash_port_program(uint8_t device,
                                       uint32_t address,
                                       uint32_t length,
...
uint8_t flash_port_geometry_get(uint8_t device, flash_port_geometry_t *p_geometry)
{
    ...
    bsp_flash_geometry_get(device,
                           &p_geometry->capacity,
                           &p_geometry->page_size,
                           &p_geometry->block_size,
                           &p_geometry->program_unit);
    return 1u;
}
