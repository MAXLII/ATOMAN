...
static uint8_t usart_empty_rx_get_byte(uint8_t *p_data)
{
    (void)p_data;
    return 0u;
}
...
REG_LINK(USART2_LINK, s_usart_null_tx_func, usart_empty_rx_get_byte, NULL, 0u);
REG_LINK(UART4_LINK, s_usart_null_tx_func, usart_empty_rx_get_byte, NULL, 0u);
