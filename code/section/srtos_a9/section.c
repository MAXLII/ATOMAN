...
static void task_context_release(reg_task_t *p_task)
{
         ...
         || (((snapshot_address - pool_start) % sizeof(uint32_t)) != 0u)) /* Snapshot is not word aligned. */
    {
        ...
        g_section_scheduler_debug.invariant_fail_count++;
        task_fault_set(SECTION_TASK_FAULT_CONTEXT_POOL_CORRUPT, p_task, p_task->p_sp, p_task->snapshot_capacity_words);
        return;
    }
    ...
    {
        g_section_fault_debug.task_context_release_fail_count++;
        task_fault_set(SECTION_TASK_FAULT_CONTEXT_RELEASE_ORDER, p_task, p_task->p_sp, p_task->snapshot_capacity_words);
        return;
    }
...
static void task_context_release_gap_if_head(void)
{
         ...
         && (task_context_pool_head == task_context_pool_gap_start))
    {
        ...
        {
            g_section_scheduler_debug.invariant_fail_count++;
            task_fault_set(SECTION_TASK_FAULT_CONTEXT_POOL_CORRUPT, NULL, NULL, task_context_pool_gap_words);
            return;
        }
    ...
    if (task_context_pool_gap_words == 0u)
    {
        if (task_context_pool_gap_start != 0u)
        {
            valid = 0u;
              ...
              || ((task_context_pool_gap_start + task_context_pool_gap_words) != SECTION_TASK_CONTEXT_POOL_WORDS)
...
static void task_stack_prepare_initial(reg_task_t *p_task)
{
    ...
    {
        task_fault_set(SECTION_TASK_FAULT_RUNTIME_STACK_TOO_SMALL, p_task, NULL, TASK_A9_CONTEXT_WORDS);
        return;
    }
...
static reg_task_t *task_stack_pick_next(void)
{
    ...
    {
        p_candidate = srtos_task_ready_pop_unlocked(&p_srtos_task_unfinished_first, &p_srtos_task_unfinished_tail);
        task_ready_pick_burst_count = 0u;
...
uint32_t section_task_switch_pending(void)
{
    ...
    if (    (p_srtos_task_ready_first != NULL)
         || (p_srtos_task_unfinished_first != NULL))
    {
        pending = 1u;
    }
...
static void section_task_entry(void)
{
    for (;;)
    {
        ...
        if (    (p_task_current == NULL)
             || (p_task_current->state != (uint8_t)TASK_STACK_STATE_RUNNING))
        {
...
void section_init(void)
{
    ...
    for (const reg_section_t *p_entry = SECTION_REG_FIRST; p_entry < SECTION_REG_LAST; ++p_entry)
    {
...
static void link_process(section_link_t *p_link)
{
    ...
    while (processed_byte_count < SECTION_LINK_RX_BYTE_BUDGET)
    {
        ...
        {
            const section_link_handler_item_t *p_handler = &p_link->handler_arr[handler_index]; /* Handler bound to the current Link. */
