...
{
    atomic_flag_clear_explicit(&pool_lock, memory_order_release);
}
...
{
    if (    (device >= 2u)
         || (command < DEMO_STORAGE_SCAN)
         || (command > DEMO_STORAGE_DEFAULTS))
    {
        return 0u;
    }
...
uint8_t demo_storage_exchange_finish(uint32_t id,
                                     int32_t result,
                                     const demo_storage_params_t *p_restored,
                                     const demo_storage_device_t *p_devices)
{
    if (    (p_devices == NULL)
         || (enter() == 0u))
    {
