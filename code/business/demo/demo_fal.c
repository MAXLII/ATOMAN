...
static void calculate(void)
{
    ...
    if (    (snapshot.params.input_mv > 100000u)
         || (snapshot.params.gain_milli > 10000u)
...
static void show(DEC_MY_PRINTF)
{
    ...
    my_printf->my_printf("demo output_mv=%lu threshold_hit=%lu\r\n",
                         (unsigned long)output_mv,
                         (unsigned long)threshold_hit);

    for (uint32_t i = 0u; i < 2u; i++)
    {
        const demo_storage_device_t *p_view = &snapshot.devices[i]; /* 当前介质快照。 */
        my_printf->my_printf(
            "dev=%lu id=%08lX online=%u valid=%u seq=%lu page=%lu block=%lu error=%ld ecc=%lu bad=%lu\r\n",
            (unsigned long)i,
            (unsigned long)p_view->chip_id,
            (unsigned)p_view->online,
            (unsigned)p_view->valid,
            (unsigned long)p_view->sequence,
            (unsigned long)p_view->page_size,
            (unsigned long)p_view->block_size,
            (long)p_view->driver_error,
            (unsigned long)p_view->corrected_bits,
            (unsigned long)p_view->bad_blocks);
        my_printf->my_printf("  log seq=%lu input=%lu gain=%lu threshold=%lu\r\n",
                             (unsigned long)p_view->log_sequence,
                             (unsigned long)p_view->last_log.input_mv,
                             (unsigned long)p_view->last_log.gain_milli,
                             (unsigned long)p_view->last_log.threshold_mv);

        for (uint32_t z = 0u; z < 5u; z++)
        {
            my_printf->my_printf("  %s offset=%08lX size=%lu write=%u\r\n",
                                 names[z],
                                 (unsigned long)p_view->zones[z].offset,
                                 (unsigned long)p_view->zones[z].size,
...
static void apply(DEC_MY_PRINTF)
{
    demo_storage_params_t params = { /* 一次性发布全部参数，不暴露池内地址。 */
                                    .input_mv     = edit_input_mv,
                                    .gain_milli   = edit_gain_milli,
                                    .threshold_mv = edit_threshold_mv};
...
static void scan(DEC_MY_PRINTF)
{
    report_request(DEMO_STORAGE_SCAN, my_printf);
}
static void save(DEC_MY_PRINTF)
{
    report_request(DEMO_STORAGE_SAVE, my_printf);
}
static void load(DEC_MY_PRINTF)
{
    report_request(DEMO_STORAGE_LOAD, my_printf);
}
static void log_append(DEC_MY_PRINTF)
{
    report_request(DEMO_STORAGE_LOG, my_printf);
}
static void defaults(DEC_MY_PRINTF)
{
    report_request(DEMO_STORAGE_DEFAULTS, my_printf);
}
static void test(DEC_MY_PRINTF)
{
    ...
    {
        test_armed = 0u;
        report_request(DEMO_STORAGE_TEST, my_printf);
    }
    else if (    (my_printf != NULL)
              && (my_printf->my_printf != NULL))
    {
