...
_Static_assert(    DEMO_FAL_LOG_BLOCKS >= 1u
                    && DEMO_FAL_LOG_BLOCKS <= 253u,
...
static fal_result_t result_map(FLASH_PORT_RESULT_E result)
{
    switch (result)
    {
    case FLASH_PORT_SUCCESS:
        return FAL_RESULT_SUCCESS;
    case FLASH_PORT_INVALID_ARGUMENT:
...
static fal_result_t flash_init(void *p_context)
{
    ...
    if (flash_port_geometry_get(device, &geometry) == 0u)
    {
...
static fal_device_state_t flash_state(void *p_context)
{
    ...
    {
    case FLASH_PORT_READY:
        return FAL_DEVICE_STATE_READY;
    case FLASH_PORT_BUSY:
...
static fal_result_t flash_read(void *p_context,
                               uint32_t address,
                               uint32_t length,
                               uint8_t *p_data)
{
...
static fal_result_t flash_program(void *p_context,
                                  uint32_t address,
                                  uint32_t length,
...
static const fal_zone_cfg_t nor_zones[] = {
    /* NOR 分区按物理地址顺序排列。 */
    ...
    {.zone_id     = DEMO_FAL_NOR_PARAM_A,
     .size        = DEMO_FAL_PARAM_BLOCKS * DEMO_FAL_NOR_BLOCK_SIZE,
     .permissions = FAL_ZONE_PERMISSION_ALL},
    {.zone_id     = DEMO_FAL_NOR_PARAM_B,
     .size        = DEMO_FAL_PARAM_BLOCKS * DEMO_FAL_NOR_BLOCK_SIZE,
     .permissions = FAL_ZONE_PERMISSION_ALL},
    {.zone_id     = DEMO_FAL_NOR_TEST,
     .size        = DEMO_FAL_TEST_BLOCKS * DEMO_FAL_NOR_BLOCK_SIZE,
     .permissions = FAL_ZONE_PERMISSION_ALL},
    {.zone_id     = DEMO_FAL_NOR_LOG,
     .size        = DEMO_FAL_LOG_BLOCKS * DEMO_FAL_NOR_BLOCK_SIZE,
     .permissions = FAL_ZONE_PERMISSION_ALL},
};
static const fal_zone_cfg_t nand_zones[] = {
    /* NAND 地址只包含主区。 */
    {.zone_id = DEMO_FAL_NAND_PROTECTED,
     .size = DEMO_FAL_NAND_CAPACITY - DEMO_FAL_USED_BLOCKS * DEMO_FAL_NAND_BLOCK_SIZE,
     .permissions = FAL_ZONE_PERMISSION_READ},
    {.zone_id     = DEMO_FAL_NAND_PARAM_A,
     .size        = DEMO_FAL_PARAM_BLOCKS * DEMO_FAL_NAND_BLOCK_SIZE,
     .permissions = FAL_ZONE_PERMISSION_ALL},
    {.zone_id     = DEMO_FAL_NAND_PARAM_B,
     .size        = DEMO_FAL_PARAM_BLOCKS * DEMO_FAL_NAND_BLOCK_SIZE,
     .permissions = FAL_ZONE_PERMISSION_ALL},
    {.zone_id     = DEMO_FAL_NAND_TEST,
     .size        = DEMO_FAL_TEST_BLOCKS * DEMO_FAL_NAND_BLOCK_SIZE,
     .permissions = FAL_ZONE_PERMISSION_ALL},
    {.zone_id     = DEMO_FAL_NAND_LOG,
     .size        = DEMO_FAL_LOG_BLOCKS * DEMO_FAL_NAND_BLOCK_SIZE,
     .permissions = FAL_ZONE_PERMISSION_ALL},
};
static const fal_device_cfg_t devices[DEMO_FAL_DEVICE_COUNT] = {
    /* 设备注册表，通过适配函数注册 Interface 操作。 */
    {.device_id         = 1u,
     .capacity          = DEMO_FAL_NOR_CAPACITY,
     .program_page_size = 256u,
     .erase_block_size  = DEMO_FAL_NOR_BLOCK_SIZE,
     .max_read_size     = 256u,
     .p_zones           = nor_zones,
     .zone_count        = DEMO_FAL_ZONE_COUNT,
     .program_unit_size = 1u,
     .ops = {.p_context   = &device_indices[0],
             .p_init      = flash_init,
             .p_get_state = flash_state,
             .p_read      = flash_read,
             .p_program   = flash_program,
             .p_erase     = flash_erase,
             .p_sync      = NULL}},
    {.device_id         = 2u,
     .capacity          = DEMO_FAL_NAND_CAPACITY,
     .program_page_size = 2048u,
     .erase_block_size  = DEMO_FAL_NAND_BLOCK_SIZE,
     .max_read_size     = 2048u,
     .p_zones           = nand_zones,
     .zone_count        = DEMO_FAL_ZONE_COUNT,
     .program_unit_size = 2048u,
     .ops = {.p_context   = &device_indices[1],
             .p_init      = flash_init,
             .p_get_state = flash_state,
             .p_read      = flash_read,
             .p_program   = flash_program,
...
const fal_cfg_t g_demo_fal_cfg[DEMO_FAL_DEVICE_COUNT] = {
    /* 两个实例独立挂载，故障互不阻塞。 */
    {.p_devices = &devices[DEMO_FAL_NOR], .device_count = 1u},
    {.p_devices = &devices[DEMO_FAL_NAND], .device_count = 1u},
};
const fal_runtime_t g_demo_fal_runtime = {
    /* 只描述绑定关系，生命周期和遍历由 Core 执行。 */
...
{
    if (device >= DEMO_FAL_DEVICE_COUNT)
    {
        return NULL;
    }
