...
static npc_ctrl_setpoint_t active_setpoint = {.vd_pos_slew_vps = NPC_CFG_DEFAULT_VD_POS_SLEW_VPS,
                                              .freq_hz         = 50.0f,
                                              .v_dc_half_min   = 20.0f,
                                              .run_allowed     = 0u};
...
static void npc_ctrl_reset_states(void)
{
    ...
    (void)memset(v_residual_lowpass, 0, sizeof(v_residual_lowpass)); /* 重启时清除电压残差滤波历史。 */
...
static void FUNC_RAM npc_ctrl_run(void)
{
    ...
    park(voltage_observer.output.alpha_pos, voltage_observer.output.beta_pos, sine, cosine, &v_dq[0], &v_dq[1]);  /* 正序电压采用 theta。 */
    park(voltage_observer.output.alpha_neg, voltage_observer.output.beta_neg, -sine, cosine, &v_dq[2], &v_dq[3]); /* 负序电压采用 -theta。 */
    park(current_observer.output.alpha_pos, current_observer.output.beta_pos, sine, cosine, &i_dq[0], &i_dq[1]);  /* 正序电流采用 theta。 */
    park(current_observer.output.alpha_neg, current_observer.output.beta_neg, -sine, cosine, &i_dq[2], &i_dq[3]); /* 负序电流采用 -theta。 */
    ...
    inv_clarke(current_alpha, current_beta, &i_fundamental[0], &i_fundamental[1], &i_fundamental[2]); /* 重构三相基波电流。 */
    /* 残差先滤除开关频率分量，再形成低频负反馈，避免直接反馈纹波激发延迟环路。 */
    i_bias_ab[0] += bias_filter_coeff * (i_ab[0] - current_alpha - i_bias_ab[0]);
    i_bias_ab[1] += bias_filter_coeff * (i_ab[1] - current_beta - i_bias_ab[1]);
    ...
    inv_park(voltage_ref_raw[0], voltage_ref_raw[1], sine, cosine, &voltage_pos_ab[0], &voltage_pos_ab[1]);  /* 正序电压返回静止坐标系。 */
    inv_park(voltage_ref_raw[2], voltage_ref_raw[3], -sine, cosine, &voltage_neg_ab[0], &voltage_neg_ab[1]); /* 负序电压返回静止坐标系。 */
    ...
    voltage_residual[0] = v_ab[0] - voltage_observer.output.alpha_pos - voltage_observer.output.alpha_neg;
    voltage_residual[1] = v_ab[1] - voltage_observer.output.beta_pos - voltage_observer.output.beta_neg;

    for (uint32_t axis = 0u; axis < 2u; ++axis) /* 更新低通状态，再相减得到高通补偿。 */
    {
        v_residual_lowpass[axis] += damping_filter_coeff * (voltage_residual[axis] - v_residual_lowpass[axis]);
        v_damping_ab[axis] = ctrl_cfg.voltage_damping_gain * (voltage_residual[axis] - v_residual_lowpass[axis]);
    }
