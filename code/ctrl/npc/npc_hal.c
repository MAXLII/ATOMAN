...
static npc_fsm_hal_t fsm_hal =
{
    .p_enter_run_func = npc_hal_enter_run,
    .p_exit_run_func = npc_hal_exit_run
};
...
void npc_hal_hard_protect_trip(uint32_t fault, uint32_t phase)
{
        ...
        (phase < 3u))      /* The upper byte carries a valid phase index. */
    {
        (void)atomic_compare_exchange_strong(
            &fault_word, &expected, (unsigned int)((phase << 8u) | fault)); /* Preserve the first latched cause. */
    }
}
