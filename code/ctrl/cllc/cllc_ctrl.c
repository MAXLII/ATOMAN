...
static void initialize_control_sets(void)
{
    ...
    forward_ok = pi_dual_compete_init(&forward_compete,
    ...
    forward_pr_ok = pr_init(&forward_voltage_pr,
    ...
    reverse_ok = pi_tustin_init(&reverse_voltage_loop,
...
static void run_forward(float ctrl_ts)
{
    ...
    battery_voltage_ref_v = slew_to(battery_voltage_ref_v, p_active_setpoint->battery_voltage_ref_v, reference_step_v);
    battery_current_ref_a = p_active_setpoint->battery_current_limit_a;
...
static void run_reverse(float ctrl_ts)
{
    ...
    forward_pr_output = 0.0f;
    bus_voltage_ref_v = slew_to(bus_voltage_ref_v, p_active_setpoint->bus_voltage_ref_v, reference_step_v);
