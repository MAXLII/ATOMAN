...
static bool inv_ctrl_update_filter_frequency(float freq_hz)
{
    ...
    if (omega_new != omega)
    {
        ...
        update_ok = apf_update_frequency(&inductor_current_apf, omega)
                 && update_ok;
        update_ok = resonant_update_frequency(&harmonic_3, omega)
                 && update_ok;
        update_ok = resonant_update_frequency(&harmonic_5, omega)
                 && update_ok;
        update_ok = resonant_update_frequency(&harmonic_7, omega)
                 && update_ok;
    }
...
static void inv_ctrl_reinit_states(void)
{
    ...
    if (resonant_init(&harmonic_3, INV_CTRL_HARMONIC_3_GAIN, INV_CTRL_HARMONIC_3_ORDER, omega, ctrl_ts) == false)
    {
        init_ok = false;
    }

    if (resonant_init(&harmonic_5, INV_CTRL_HARMONIC_5_GAIN, INV_CTRL_HARMONIC_5_ORDER, omega, ctrl_ts) == false)
    {
        init_ok = false;
    }

    if (resonant_init(&harmonic_7, INV_CTRL_HARMONIC_7_GAIN, INV_CTRL_HARMONIC_7_ORDER, omega, ctrl_ts) == false)
    {
        init_ok = false;
    }
...
static void inv_ctrl_isr(void)
{
    ...
    {
        RAMP(freq_hz_ramped, p_setpoint->freq_hz, p_setpoint->freq_slew_hzps * inv_cfg_get_ctrl_ts());
    }
    else
    {
    ...
    DQ_CAL(v_cap_fb, v_cap_beta, sintheta, costheta, v_d_act, v_q_act);

    i_l_beta = apf_cal(&inductor_current_apf, i_l_fb);
    DQ_CAL(i_l_fb, i_l_beta, sintheta, costheta, i_l_d_act, i_l_q_act);
    ...
    else
    {
        ...
        i_cap_beta_act = HW_AC_SIDE_CAP_VALUE * (v_cap_beta - v_cap_beta_last) / inv_cfg_get_ctrl_ts();
    }
    v_cap_last      = v_cap_fb;
    v_cap_beta_last = v_cap_beta;
    DQ_CAL(i_cap_act, i_cap_beta_act, sintheta, costheta, i_cap_d_act, i_cap_q_act);

    (void)pi_tustin_cal(&volt_loop_d);
    (void)pi_tustin_cal(&volt_loop_q);

    /* 由 i_load=iL-iC 估算负载电流，并加入电容旋转项得到电感电流给定。 */
    i_l_d_ref = (i_l_d_act - i_cap_d_act) + volt_loop_d.output.val - (omega * HW_AC_SIDE_CAP_VALUE * v_q_act);
    i_l_q_ref = (i_l_q_act - i_cap_q_act) + volt_loop_q.output.val + (omega * HW_AC_SIDE_CAP_VALUE * v_d_act);
    ...
    inductor_decouple_d = (INV_CTRL_FILTER_IND_RES_OHM * i_l_d_act) - (omega * HW_AC_SIDE_IND_VALUE * i_l_q_act);
    inductor_decouple_q = (INV_CTRL_FILTER_IND_RES_OHM * i_l_q_act) + (omega * HW_AC_SIDE_IND_VALUE * i_l_d_act);
    inductor_decouple_alpha = (costheta * inductor_decouple_d) - (sintheta * inductor_decouple_q);
