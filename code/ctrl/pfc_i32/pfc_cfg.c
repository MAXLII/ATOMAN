...
void pfc_cfg_set_vbus_ref_v(float vbus_ref_v)
{
    if (pfc_cfg_setpoint_mgr.building.p_data != NULL)
    {
        pfc_cfg_setpoint_mgr.building.p_data->vbus_ref = PFC_CTRL_VBUS_REF_TO_CODE(vbus_ref_v);
    }
}
...
void pfc_cfg_set_vbus_slew_vps(float vbus_slew_vps)
{
    if (pfc_cfg_setpoint_mgr.building.p_data != NULL)
    {
        pfc_cfg_setpoint_mgr.building.p_data->vbus_slew_code_per_s = PFC_CTRL_VBUS_SLEW_TO_CODE_PER_S(vbus_slew_vps);
    }
}
