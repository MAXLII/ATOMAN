...
#define BOOST_CTRL_DCM_CURR_REF_DIV_SHIFT     (BOOST_CTRL_K2_IND_CURR_FB_SHIFT - BOOST_CTRL_DCM_CURR_REF_FRAC_SHIFT)
...
#define BOOST_CTRL_DCM_CODE_NORM                                                            \
    ((float)BOOST_CTRL_CMP_MAX                                                              \
     / (((float)BOOST_CTRL_IND_CURR_LOOP_REF_CODE_MAX / BOOST_CTRL_IND_CURR_LOOP_REF_MAX_A) \
        * (BOOST_CTRL_OUT_VOLT_LOOP_REF_MAX_V / (float)BOOST_CTRL_OUT_VOLT_LOOP_REF_CODE_MAX)))
...
static inline int32_t calc_cmp(int32_t v_l_cmd, int32_t v_in, int32_t v_out)
{
    ...
    numerator_i64 = ((int64_t)v_in * (int64_t)BOOST_CTRL_OUT_VOLT_LOOP_V_OUT_FF_K) - (int64_t)v_l_cmd;
...
static void calc_dcm_duty_const(void)
{
    ...
    const_val = (float)BOOST_CTRL_DCM_CONST_SCALE * sqrtf(((2.0f * BOOST_HW_IND) / pwm_ts) * BOOST_CTRL_DCM_CODE_NORM);
                                         ...
                                         int32_t duty_ccm)
{
    ...
    sqrt_input  = mul_div_u32_sat_i32((uint32_t)i_ref, (uint32_t)duty_ccm, (uint32_t)v_in);
    sqrt_result = (int32_t)sqrtf((float)sqrt_input);

    return mul_div_u32_sat_i32((uint32_t)const_k, (uint32_t)sqrt_result, (uint32_t)BOOST_CTRL_DCM_CONST_SCALE);
}
...
static void FUNC_RAM boost_isr(void)
{
         ...
         || (v_out <= 0))
    {
        for (ch = 0U; ch < BOOST_CTRL_IND_CURR_CH_NUM; ch++)
        {
            p_hal_isr->p_set_pwm_func[ch]((uint32_t)BOOST_CTRL_CMP_MIN, freewheel_en, main_en);
        }
    ...
    for (ch = 0U; ch < BOOST_CTRL_IND_CURR_CH_NUM; ch++)
    {
        ...
        ccm_cmp = cmp;
        dcm_cmp = calc_dcm_duty_code(dcm_duty_const, dcm_curr_ref, v_in, BOOST_CTRL_CMP_MAX - ccm_cmp);
        ...
        p_hal_isr->p_set_pwm_func[ch]((uint32_t)cmp, freewheel_en, main_en);
    }
}
...
static void boost_task(void)
{
    ...
    out_curr_i_l_lmt = scale_ind_curr_to_k2(div_by_cmp_i32(p_ctrl_active_setpoint->out_curr_lmt, cmp));
