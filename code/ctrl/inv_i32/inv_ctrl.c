...
static bool apply_coeff_bundle(const inv_ctrl_coeff_bundle_t *p_bundle)
{
    ...
    valid = resonant_i32_set_coeff(&harmonic_3, &p_bundle->harmonic_3)
         && valid;
    valid = resonant_i32_set_coeff(&harmonic_5, &p_bundle->harmonic_5)
         && valid;
    valid = resonant_i32_set_coeff(&harmonic_7, &p_bundle->harmonic_7)
         && valid;
    return valid;
}
...
static void reinit_states(void)
{
    ...
    valid = apf_i32_init(&volt_apf, &initial_coeff.apf)
         && valid;
    valid = resonant_i32_init(&harmonic_3, &initial_coeff.harmonic_3)
         && valid;
    valid = resonant_i32_init(&harmonic_5, &initial_coeff.harmonic_5)
         && valid;
    valid = resonant_i32_init(&harmonic_7, &initial_coeff.harmonic_7)
         && valid;
    ...
    cap_diff_gain = HW_AC_SIDE_CAP_VALUE * (float)inv_cfg_get_ctrl_freq_hz() * INV_CTRL_IND_CURR_CODE_PER_A
                  / INV_CTRL_AC_VOLT_CODE_PER_V * (float)(1UL << INV_CTRL_CAP_DIFF_GAIN_Q_SHIFT);
...
static void inv_ctrl_isr(void)
{
    ...
    else
    {
        ...
        freq_hz_ramped_q16 = ramp_u32(freq_hz_ramped_q16, limit_frequency_q16(p_setpoint->freq_hz_q16), freq_step_q16);
    }
    ...
    v_ref_peak_ramped_q16 = ramp_i32(v_ref_peak_ramped_q16, p_setpoint->v_ref_peak_code_q16, ref_step_q16);
    v_d_ref = v_ref_peak_ramped_q16 >> INV_CTRL_REF_Q_SHIFT;
    v_q_ref = 0;

    v_cap_beta = apf_i32_cal(&volt_apf, v_cap_fb);
    accumulator = ((int64_t)v_cap_fb * (int64_t)cos_theta_q15) + ((int64_t)v_cap_beta * (int64_t)sin_theta_q15);
    v_d_act = dq_shift(accumulator);
    accumulator = -((int64_t)v_cap_fb * (int64_t)sin_theta_q15) + ((int64_t)v_cap_beta * (int64_t)cos_theta_q15);
    ...
    accumulator = (int64_t)resonant_i32_cal(&harmonic_3, v_cap_fb) + (int64_t)resonant_i32_cal(&harmonic_5, v_cap_fb)
