...
static inv_ctrl_setpoint_t setpoint_building = {
    ...
    .v_ref_slew_code_q16_per_s = INV_CTRL_RMS_SLEW_TO_PEAK_CODE_Q16_PER_S(INV_CFG_DEFAULT_RMS_SLEW_VPS),
...
void inv_cfg_set_timing(const inv_ctrl_timing_t *p_timing)
{
    ...
    phase_gain = (65536.0f / p_timing->ctrl_freq) * (float)(1UL << INV_CTRL_PHASE_GAIN_Q_SHIFT);
...
void inv_cfg_set_freq_slew_hzps(float freq_slew_hzps)
{
    if (setpoint_mgr.building.p_data != NULL)
    {
        setpoint_mgr.building.p_data->freq_slew_hzps_q16 = float_to_u32(freq_slew_hzps * (float)INV_CTRL_FREQ_Q_ONE);
    }
}
...
void inv_cfg_set_rms_ref_v(float rms_ref_v)
{
    ...
    peak_code_q16 = rms_ref_v * INV_CTRL_SQRT2 * INV_CTRL_AC_VOLT_CODE_PER_V * (float)INV_CTRL_REF_Q_ONE;
...
void inv_cfg_set_rms_slew_vps(float rms_slew_vps)
{
    ...
    slew_code_q16 = rms_slew_vps * INV_CTRL_SQRT2 * INV_CTRL_AC_VOLT_CODE_PER_V * (float)INV_CTRL_REF_Q_ONE;
