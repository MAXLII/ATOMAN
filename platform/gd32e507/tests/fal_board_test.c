...
static uint8_t healthy(void)
{
    ...
    if (check((int32_t)(health.chip_id == 0x00C84015u), 1, 101u) == 0u)
    {
        return 0u;
    }

    if (check(health.error, 0, 102u) == 0u)
    {
        return 0u;
    }
    flash_port_health_get(1u, &health);

    if (check((int32_t)(health.chip_id == 0xC8F1801Du), 1, 103u) == 0u)
    {
        return 0u;
    }

    if (check(health.error, 0, 104u) == 0u)
    {
        return 0u;
    }
...
static void demo_case(void)
{
    ...
    if (id >= 4u)
    {
        device = (uint8_t)(g_fal_board_report.iteration % 2u);
    }
    ...
    if (step == 0u)
    {
        ...
        if (check((int32_t)demo_storage_business_request(device, DEMO_STORAGE_TEST), 1, 201u) == 0u)
        {
            return;
        }
        ...
        if (id == 5u)
        {
            if (check((int32_t)demo_storage_business_request((uint8_t)(1u - device), DEMO_STORAGE_TEST), 0, 501u) == 0u)
            {
                return;
            }
        }

        if (check((int32_t)demo_storage_business_read(&snapshot), 1, 202u) == 0u)
        {
            return;
        }
    ...
    if (check((int32_t)(snapshot.completed_id == request_id), 1, 203u) == 0u)
    {
        return;
    }

    if (check(snapshot.result, 0, 204u) == 0u)
    {
        return;
    }
    ...
    }
    {
        ...
        if (elapsed > g_fal_board_report.max_demo_us)
        {
            g_fal_board_report.max_demo_us = elapsed;
        }
    }
...
static void nand_case(void)
{
    ...
    if (step == 0u)
    {
        ...
        if (check((int32_t)flash_port_erase(1u, TEST_NAND_BASE, 131072u), 0, 601u) == 0u)
        {
            return;
        }
        step = 1u;
    ...
    if (state == FLASH_PORT_BUSY)
    {
        return;
    }

    if (check((int32_t)state, (int32_t)FLASH_PORT_READY, 602u) == 0u)
    {
        return;
    }
    ...
    if (step == 1u)
    {
        if (check((int32_t)flash_port_program(1u, TEST_NAND_BASE, TEST_PAGE, data), 0, 603u) == 0u)
        {
            return;
        }

        if (check((int32_t)flash_port_program(1u, TEST_NAND_BASE, TEST_PAGE, replacement), (int32_t)FLASH_PORT_INVALID_ARGUMENT, 604u) == 0u)
        {
            return;
        }
        step = 2u;
    ...
    if (step == 2u)
    {
        if (check((int32_t)flash_port_read(1u, TEST_NAND_BASE, TEST_PAGE, readback), 0, 605u) == 0u)
        {
            return;
        }

        if (check((int32_t)flash_port_read(1u, TEST_NAND_BASE, TEST_PAGE, replacement), (int32_t)FLASH_PORT_INVALID_ARGUMENT, 606u) == 0u)
        {
            return;
        }
        step = 3u;
    ...
    if (check((int32_t)memcmp(data, readback, TEST_PAGE), 0, 607u) == 0u)
    {
        return;
    }
    ...
    for (uint32_t i = 0u; i < TEST_PAGE; i++)
    {
        if (check((int32_t)replacement[i], 0xA5, 608u) == 0u)
        {
            return;
        }
    }
...
{
    if (check((int32_t)fal_erase(&g_demo_fal[0], DEMO_FAL_NOR_PROTECTED, 0u, 4096u), (int32_t)FAL_RESULT_PERMISSION_DENIED, 701u) == 0u)
    {
        return;
    }

    if (check((int32_t)fal_erase(&g_demo_fal[1], DEMO_FAL_NAND_PROTECTED, 0u, 131072u), (int32_t)FAL_RESULT_PERMISSION_DENIED, 702u) == 0u)
    {
        return;
    }

    if (check((int32_t)fal_write(&g_demo_fal[0], DEMO_FAL_NOR_PROTECTED, 0u, 256u, data), (int32_t)FAL_RESULT_PERMISSION_DENIED, 703u) == 0u)
    {
        return;
    }

    if (check((int32_t)fal_write(&g_demo_fal[1], DEMO_FAL_NAND_PROTECTED, 0u, TEST_PAGE, data), (int32_t)FAL_RESULT_PERMISSION_DENIED, 704u) == 0u)
    {
        return;
    }

    if (check((int32_t)fal_read(&g_demo_fal[0], DEMO_FAL_NOR_TEST, 8192u, 1u, readback), (int32_t)FAL_RESULT_OUT_OF_RANGE, 705u) == 0u)
    {
        return;
    }

    if (check((int32_t)fal_read(&g_demo_fal[1], DEMO_FAL_NAND_TEST, 262144u, 1u, readback), (int32_t)FAL_RESULT_OUT_OF_RANGE, 706u) == 0u)
    {
        return;
    }

    if (check((int32_t)fal_write(&g_demo_fal[1], DEMO_FAL_NAND_TEST, 1u, TEST_PAGE, data), (int32_t)FAL_RESULT_INVALID_ARGUMENT, 707u) == 0u)
    {
        return;
    }

    if (check((int32_t)fal_is_busy(&g_demo_fal[0]), 0, 708u) == 0u)
    {
        return;
    }

    if (check((int32_t)fal_is_busy(&g_demo_fal[1]), 0, 709u) == 0u)
    {
        return;
    }
...
static void concurrent_case(void)
{
    ...
    if (step == 0u)
    {
        if (check((int32_t)fal_erase(&g_demo_fal[0], DEMO_FAL_NOR_TEST, 0u, 4096u), (int32_t)FAL_RESULT_IN_PROGRESS, 801u) == 0u)
        {
            return;
        }
        step = 1u;
    ...
    if (step == 1u)
    {
        ...
        if (check((int32_t)fal_result_get(&g_demo_fal[0]), 0, 802u) == 0u)
        {
            return;
        }

        if (check((int32_t)fal_write(&g_demo_fal[0], DEMO_FAL_NOR_TEST, 0u, TEST_PAGE, data), (int32_t)FAL_RESULT_IN_PROGRESS, 803u) == 0u)
        {
            return;
        }

        if (check((int32_t)fal_read(&g_demo_fal[1], DEMO_FAL_NAND_TEST, 0u, TEST_PAGE, readback), (int32_t)FAL_RESULT_IN_PROGRESS, 804u) == 0u)
        {
            return;
        }

        if (check((int32_t)fal_is_busy(&g_demo_fal[0]), 1, 805u) == 0u)
        {
            return;
        }

        if (check((int32_t)fal_is_busy(&g_demo_fal[1]), 1, 806u) == 0u)
        {
            return;
        }
        step = 2u;
    ...
    if (step == 2u)
    {
        ...
        if (check((int32_t)independent, 1, 807u) == 0u)
        {
            return;
        }

        if (check((int32_t)fal_result_get(&g_demo_fal[0]), 0, 808u) == 0u)
        {
            return;
        }

        if (check((int32_t)fal_result_get(&g_demo_fal[1]), 0, 809u) == 0u)
        {
            return;
        }

        if (check((int32_t)memcmp(data, readback, TEST_PAGE), 0, 810u) == 0u)
        {
            return;
        }

        if (check((int32_t)fal_read(&g_demo_fal[0], DEMO_FAL_NOR_TEST, 0u, TEST_PAGE, readback), (int32_t)FAL_RESULT_IN_PROGRESS, 811u) == 0u)
        {
            return;
        }
        step = 3u;
    ...
    if (check((int32_t)fal_result_get(&g_demo_fal[0]), 0, 812u) == 0u)
    {
        return;
    }

    if (check((int32_t)memcmp(data, readback, TEST_PAGE), 0, 813u) == 0u)
    {
        return;
    }
...
{
    if (g_fal_board_report.status >= 2u)
    {
        return;
    }

    if (demo_storage_business_read(&snapshot) == 0u)
    {
        return;
    }
    ...
    if (g_fal_board_report.case_id == 6u)
    {
        nand_case();
    }
    else if (g_fal_board_report.case_id == 7u)
    {
        boundary_case();
    }
    else if (g_fal_board_report.case_id == 8u)
    {
        concurrent_case();
    }
    else
