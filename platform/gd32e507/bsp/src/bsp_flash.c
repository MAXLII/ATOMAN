...
static const bsp_flash_chip_t *const chips[BSP_FLASH_COUNT] = { /* Indexed physical drivers. */
                                                               &bsp_nor_chip,
                                                               &bsp_nand_chip};
void bsp_flash_geometry_get(uint8_t device,
                            uint32_t *p_capacity,
                            uint32_t *p_page,
                            uint32_t *p_block,
                            uint32_t *p_unit)
...
void bsp_flash_health_get(uint8_t device, bsp_flash_health_t *p_health)
{
    if (    (device < BSP_FLASH_COUNT)
         && (p_health != NULL))
    {
        *p_health = *chips[device]->p_health;
    }
}
...
BSP_FLASH_RESULT_E bsp_flash_read(uint8_t device,
                                  uint32_t address,
                                  uint32_t length,
                                  uint8_t *p_data)
{
...
BSP_FLASH_RESULT_E bsp_flash_program(uint8_t device,
                                     uint32_t address,
                                     uint32_t length,
