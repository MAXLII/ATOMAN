...
uint8_t bsp_enet_init(void)
{
    ...
    init_status = enet_init(ENET_AUTO_NEGOTIATION, ENET_AUTOCHECKSUM_DROP_FAILFRAMES, ENET_BROADCAST_FRAMES_PASS);
                                         ...
                                         uint16_t response_capacity)
{
    ...
    if (response_append_text(p_response, response_capacity, &response_length, "FRAME_DEVICE_V1;name=" BSP_ENET_DISCOVERY_DEVICE_NAME ";ip=") == 0u)
    {
        return 0u;
    }

    if (response_append_u16(p_response, response_capacity, &response_length, ip4_addr1(p_local_address)) == 0u)
    {
        return 0u;
    }
    ...
    if (response_append_u16(p_response, response_capacity, &response_length, ip4_addr2(p_local_address)) == 0u)
    {
        return 0u;
    }
    ...
    if (response_append_u16(p_response, response_capacity, &response_length, ip4_addr3(p_local_address)) == 0u)
    {
        return 0u;
    }
    ...
    if (response_append_u16(p_response, response_capacity, &response_length, ip4_addr4(p_local_address)) == 0u)
    {
        return 0u;
    }
    ...
    if (response_append_u16(p_response, response_capacity, &response_length, ENET_CONFIG_TCP_FRAME_PORT) == 0u)
    {
        return 0u;
    }
    ...
    if (response_append_text(p_response, response_capacity, &response_length, ";fw_version=" BSP_ENET_DISCOVERY_FIRMWARE_VERSION ";protocol_version=" BSP_ENET_DISCOVERY_PROTOCOL_VERSION) == 0u)
    {
        return 0u;
    }
                                                           ...
                                                           uint16_t remote_port)
{
    ...
    copied_length = pbuf_copy_partial(p_packet, request, BSP_ENET_DISCOVERY_REQUEST_LENGTH, 0u);
    ...
    {
        send_status = udp_sendto(p_endpoint, p_response_packet, p_remote_address, remote_port);
    }
    pbuf_free(p_response_packet);

    return (send_status == ERR_OK) ? BSP_ENET_DISCOVERY_RESPONSE_SENT_E : BSP_ENET_DISCOVERY_RESPONSE_ERROR_E;
}
