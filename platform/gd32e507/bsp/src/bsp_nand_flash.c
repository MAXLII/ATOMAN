...
static void address_send(uint32_t row, uint32_t column, uint8_t with_column)
{
    if (with_column == 1u)
    {
        NAND_ADDR = (uint8_t)column;
        NAND_ADDR = (uint8_t)(column >> 8u);
    }
...
static BSP_FLASH_RESULT_E init(void)
{
    ...
    gpio_init(GPIOD,
              GPIO_MODE_AF_PP,
              GPIO_OSPEED_50MHZ,
              GPIO_PIN_0 | GPIO_PIN_1 | GPIO_PIN_14 | GPIO_PIN_15 | GPIO_PIN_11 | GPIO_PIN_12 | GPIO_PIN_4 | GPIO_PIN_5
                  | GPIO_PIN_7);
    ...
    {
        if (    ((systick_gettime_100us() - started) >= 2u)
             && ((status_read() & 0x40u) != 0u))
        {
            ready = 1u;
            break;
        }
    ...
    health.id = ((uint32_t)id[0] << 24u) | ((uint32_t)id[1] << 16u) | ((uint32_t)id[2] << 8u) | id[3];

    if (    (health.id != 0xC8F1801Du)
         || (id[4] != 0x42u))
    {
        health.error = -1;
        return BSP_FLASH_IO_ERROR;
...
static void issue(void)
{
    ...
    else if (operation == 2u)
    {
        ...
        {
            bch4_encode(&page_data[i * 512u], &page_data[NAND_PAGE + 16u + i * 7u]);
        }
        NAND_CMD = 0x80u;
...
static uint8_t page_check(void)
{
    ...
    if (flash_integrity_u32_get(&page_data[NAND_PAGE + 8u]) != 0x314C4146u)
    {
        return 0u;
    }
    ...
    for (uint32_t i = 0u; i < 4u; i++)
    {
        ...
        if ((uint32_t)corrected <= UINT32_MAX - health.corrected_bits)
        {
            health.corrected_bits += (uint32_t)corrected;
        }
    }
    return (flash_integrity_crc32(page_data, NAND_PAGE) == flash_integrity_u32_get(&page_data[NAND_PAGE + 12u])) ? 1u
                                                                                                                 : 0u;
}
...
static BSP_FLASH_STATE_E state(void)
{
    ...
    if ((status & 0x40u) == 0u)
    {
        if ((systick_gettime_100us() - started) > 10000u)
        {
            health.error = -2;
        }
    ...
    else
    {
        ...
        {
            next_page[block] = (uint8_t)((current_address / NAND_PAGE) % 64u + 1u);
        }
    }
    phase = NAND_IDLE;
    return BSP_FLASH_READY;
...
static BSP_FLASH_RESULT_E begin(uint32_t address, uint32_t length, uint8_t op)
{
    ...
    if (    (phase != NAND_IDLE)
         || (health.error != 0)
         || (address >= NAND_CAPACITY)
         || (length == 0u)
         || (length > NAND_CAPACITY - address))
    {
        return BSP_FLASH_INVALID_ARGUMENT;
    }

    if (rejected[block] == 1u)
    {
        health.error = -4;
        return BSP_FLASH_IO_ERROR;
    }
...
{
    return begin(address, length, 3u);
}
