...
{
    printf(
        " Usage: htmlgen [targetdir] [-s] [-e] [-11] [-nossi] [-ssi:<filename>] [-c] [-f:<filename>] [-m] [-svr:<name>] [-x:<ext_list>] [-xc:<ext_list>" USAGE_ARG_DEFLATE
            NEWLINE NEWLINE);
...
int main(int argc, char *argv[])
{
    ...
    for (i = 1; i < argc; i++)
    {
        ...
        if (argv[i][0] == '-')
        {
            ...
            else if (strstr(argv[i], "-defl") == argv[i])
            {
                ...
                printf("Deflating all non-SSI files with level %d (but only if size is reduced)" NEWLINE,
                       deflate_level);
#else
    ...
    fprintf(data_file,
            "#ifndef FS_FILE_FLAGS_HEADER_INCLUDED" NEWLINE "#define FS_FILE_FLAGS_HEADER_INCLUDED 1" NEWLINE
            "#endif" NEWLINE);
    /* define FS_FILE_FLAGS_HEADER_PERSISTENT to 0 if not defined (compatibility with older httpd/fs: wasn't supported back then) */
    fprintf(data_file,
            "#ifndef FS_FILE_FLAGS_HEADER_PERSISTENT" NEWLINE "#define FS_FILE_FLAGS_HEADER_PERSISTENT 0" NEWLINE
            "#endif" NEWLINE);

    /* define alignment defines */
#if ALIGN_PAYLOAD
    fprintf(data_file,
            "/* FSDATA_FILE_ALIGNMENT: 0=off, 1=by variable, 2=by include */" NEWLINE
            "#ifndef FSDATA_FILE_ALIGNMENT" NEWLINE "#define FSDATA_FILE_ALIGNMENT 0" NEWLINE "#endif" NEWLINE);
#endif
    ...
    if (deflateNonSsiFiles)
    {
        if (can_be_compressed)
        {
            if (fsize < OUT_BUF_SIZE)
            {
                ...
                mz_uint comp_flags = s_tdefl_num_probes[MZ_MIN(10, deflate_level)]
                                   | ((deflate_level <= 3) ? TDEFL_GREEDY_PARSING_FLAG : 0);
                ...
                if (out_bytes < fsize)
                {
                    ...
                    memcpy(ret_buf, s_outbuf, out_bytes);
                    {
                        ...
                        tinfl_init(&inflator);
                        dec_status = tinfl_decompress(&inflator,
                                                      (const mz_uint8 *)ret_buf,
                                                      &dec_in_bytes,
                                                      s_checkbuf,
                                                      (mz_uint8 *)next_out,
                                                      &dec_out_bytes,
                    ...
                    buf        = ret_buf;
                    *file_size = out_bytes;
                    printf(" - deflate: %d bytes -> %d bytes (%.02f%%)" NEWLINE,
                           (int)fsize,
                           (int)out_bytes,
                           (float)((out_bytes * 100.0) / fsize));
                    deflatedBytesReduced += (size_t)(fsize - out_bytes);
                    *is_compressed = 1;
                }
                else
                {
                    printf(" - uncompressed: (would be %d bytes larger using deflate)" NEWLINE,
                           (int)(out_bytes - fsize));
                }
            }
            else
...
static int file_to_exclude(const char *filename)
{
    return (exclude_list != NULL)
        && ext_in_list(filename, exclude_list);
}

static int file_can_be_compressed(const char *filename)
{
    return (ncompress_list == NULL)
        || !ext_in_list(filename, ncompress_list);
}
...
int process_file(FILE *data_file, FILE *struct_file, const char *filename)
{
    ...
    can_be_compressed = includeHttpHeader
                     &&!is_ssi
                     && file_can_be_compressed(filename);
