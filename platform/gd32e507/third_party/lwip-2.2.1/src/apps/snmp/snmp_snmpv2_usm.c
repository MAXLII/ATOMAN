...
static snmp_err_t usmusertable_get_instance(const u32_t *column,
                                            const u32_t *row_oid,
                                            u8_t row_oid_len,
 ...
 *
 */
static snmp_err_t usmusertable_get_next_instance(const u32_t *column, struct snmp_obj_id *row_oid,
                                                 struct snmp_node_instance *cell_instance)
{
    const char *engineid;
    u8_t eid_len;
    ...
    for (i = 0; i < snmpv3_get_amount_of_users(); i++)
    {
        ...
        snmp_next_oid_check(&state,
                            test_oid,
                            (u8_t)(1 + eid_len + 1 + strlen(username)),
...
static const struct snmp_table_node usmusertable =
    SNMP_TABLE_CREATE(2, usmusertable_columns, usmusertable_get_instance, usmusertable_get_next_instance,
                      usmusertable_get_value, NULL, NULL);

static const struct snmp_node *const usmuser_subnodes[] = {&usmusertable.node.node};
...
static const struct snmp_scalar_array_node usmstats_scalars =
    SNMP_SCALAR_CREATE_ARRAY_NODE(1, usmstats_scalars_nodes, usmstats_scalars_get_value, NULL, NULL);

static const struct snmp_node *const usmmibobjects_subnodes[] = {&usmstats_scalars.node.node, &usmuser_treenode.node};
static const struct snmp_tree_node usmmibobjects_treenode = SNMP_CREATE_TREE_NODE(1, usmmibobjects_subnodes);

/* --- snmpUsmMIB  ----------------------------------------------------- */
static const struct snmp_node *const snmpusmmib_subnodes[] = {&usmmibobjects_treenode.node};
