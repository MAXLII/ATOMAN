...
static option_t eap_option_list[] = {
    ...
    {"srp-use-pseudonym", o_bool, &eap_states[0].es_usepseudo, "Use pseudonym if offered one by server", 1},
#endif
    {NULL}};
#endif /* PPP_OPTIONS */
...
#if PRINTPKT_SUPPORT
static int eap_printpkt(const u_char *inp,
                        int inlen,
                        void (*)(void *arg,
                                 const char *fmt,
                                 ...),
...
static const u_char wkmodulus[] = {
    };
#endif
...
static void eap_send_success(ppp_pcb *pcb)
{
    ...
    auth_peer_success(pcb, PPP_EAP, 0, pcb->eap.es_server.ea_peer, pcb->eap.es_server.ea_peerlen);
}
#endif /* PPP_SERVER */
    ...
    case eapIdentify:
             ...
             && (pcb->eap.es_server.ea_peerlen - SRP_PSEUDO_LEN) * 3 / 4 < sizeof(secbuf))
        {
                 ...
                 && i < 5)
            {
                ...
                ppp_dbglog(
                    ("decoded pseudonym to \"%.*q\"", pcb->eap.es_server.ea_peerlen, pcb->eap.es_server.ea_peer));
            }
            else
            {
        ...
        if (get_srp_secret(pcb->eap.es_unit, pcb->eap.es_server.ea_peer, pcb->eap.es_server.ea_name, (char *)secbuf, 1) != 0)
        {
...
static void eap_send_request(ppp_pcb *pcb)
{
    ...
    case eapMD5Chall:
         ...
         */
        pcb->eap.es_challen = EAP_MIN_CHALLENGE_LENGTH + magic_pow(EAP_MIN_MAX_POWER_OF_TWO_CHALLENGE_LENGTH);
    ...
    case eapSRP3:
        ...
        if (pncrypt_setkey(0))
        {
            ...
            *optr = i;
            ...
            SHA1Update(&ctxt, pcb->eap.es_server.ea_peer, pcb->eap.es_server.ea_peerlen);
 ...
 */
static void eap_send_response(ppp_pcb *pcb,
                              u_char id,
                              u_char typenum,
                              const u_char *str,
                              int lenstr)
 ...
 */
static void eap_chap_response(ppp_pcb *pcb,
                              u_char id,
                              u_char *hash,
                              const char *name,
                              int namelen)
...
int len, id;
{
    ...
    while (len > 0)
    {
        ...
        {
            SHA1Update(&ctxt, pcb->eap.es_client.ea_name, pcb->eap.es_client.ea_namelen);
        }
        SHA1Final(dig, &ctxt);
 ...
 */
static void eap_request(ppp_pcb *pcb,
                        u_char *inp,
                        int id,
                        int len)
{
...
#endif /* USE_SRP */
        eap_send_response(pcb, id, typenum, (const u_char *)pcb->eap.es_client.ea_name, pcb->eap.es_client.ea_namelen);
        break;
    ...
    case EAPT_MD5CHAP:
        ...
        if (!get_secret(pcb, pcb->eap.es_client.ea_name, rhostname, secret, &secret_len, 0))
        {
        ...
        eap_chap_response(pcb, id, hash, pcb->eap.es_client.ea_name, pcb->eap.es_client.ea_namelen);
        break;
    ...
    case EAPT_SRP:
        ...
        case EAPSRP_SKEY:
            ...
            {
                if (get_srp_secret(pcb->eap.es_unit, pcb->eap.es_client.ea_name, pcb->eap.es_client.ea_peer, secret, 0) == 0)
                {
                    /*
        ...
        case EAPSRP_SVALIDATOR:
            ...
            else
            {
                ...
                if (    len < 0
                     || t_clientverify(tc, inp + sizeof(u32_t)) != 0)
                {
 ...
 */
static void eap_response(ppp_pcb *pcb,
                         u_char *inp,
                         int id,
                         int len)
{
    ...
    {
        ppp_dbglog(("EAP: discarding Response %d; expected ID %d", id, pcb->eap.es_server.ea_id));
        return;
    }
    ...
    case EAPT_MD5CHAP:
        ...
        if (!get_secret(pcb, rhostname, pcb->eap.es_server.ea_name, secret, &secret_len, 1))
        {
    ...
    case EAPT_SRP:
        ...
        case EAPSRP_CVALIDATOR:
            ...
            {
                ppp_error(("EAP: M1 length %d < %d", len, sizeof(u32_t) + SHA_DIGESTSIZE));
                eap_figure_next_state(pcb, 1);
                break;
            }
 ...
 */
static void eap_success(ppp_pcb *pcb,
                        u_char *inp,
                        int id,
                        int len)
{
 ...
 */
static void eap_failure(ppp_pcb *pcb,
                        u_char *inp,
                        int id,
                        int len)
{
...
static void eap_input(ppp_pcb *pcb, u_char *inp, int inlen)
{
    ...
    if (    len < EAP_HEADERLEN
         || len > inlen)
    {
        ppp_error(("EAP: packet has illegal length field %d (%d..%d)", len, EAP_HEADERLEN, inlen));
        return;
    }
    len -= EAP_HEADERLEN;
 ...
 */
static const char *const eap_codenames[] = {"Request", "Response", "Success", "Failure"};

static const char *const eap_typenames[] = {"Identity",
                                            "Notification",
                                            "Nak",
                                            "MD5-Challenge",
                                            "OTP",
                        ...
                        void *arg)
{
    ...
    if (    code >= 1
         && code <= (int)LWIP_ARRAYSIZE(eap_codenames))
    ...
    case EAP_REQUEST:
        ...
        if (    rtype >= 1
             && rtype <= (int)LWIP_ARRAYSIZE(eap_typenames))
