...
_Static_assert(ENET_COMM_TCP_TX_RING_SIZE > TCP_SND_BUF, "TCP transmit ring must retain the complete LwIP send window");
_Static_assert(ENET_COMM_TCP_TX_PUMP_BUDGET <= UINT16_MAX, "TCP pump length must fit the raw API length type");
...
static err_t tcp_receive_callback(void *p_argument,
...
static void ethernet_tx_by_dma(char *p_data, int length)
{
    ...
    if (    (p_data == NULL)
         || (length <= 0)
         || ((uint32_t)length >= ENET_COMM_TCP_TX_RING_SIZE))
    {
        return;
    }
...
REG_LINK(ETHERNET_LINK, ethernet_tx_func, ethernet_rx_get_byte, ethernet_handler_arr,
                                  ...
                                  err_t error)
{
    ...
    for (p_segment = p_packet; p_segment != NULL; p_segment = p_segment->next)
    {
        ...
        comm_run_buffer(p_bytes, (uint32_t)p_segment->len, &ethernet_tx_func, (void *)&ethernet_comm_ctx);
        comm_v1_run_buffer(p_bytes, (uint32_t)p_segment->len, &ethernet_tx_func, (void *)&ethernet_comm_v1_ctx);
    }
...
static void tcp_transmit_service(void)
{
    ...
    while (submitted_length < ENET_COMM_TCP_TX_PUMP_BUDGET)
    {
        ...
        write_status = tcp_write(p_tcp_connection, &tcp_tx_ring[submit], send_length, 0u);
...
static uint8_t stack_start(void)
{
    ...
    if (netif_add(&network_interface, &ip_address, &network_mask, &gateway_address, NULL, ethernetif_init, ethernet_input) == NULL)
    {
