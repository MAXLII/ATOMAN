...
static void platform_pwm_write(float alpha,
                               float beta,
                               float vdc_p,
                               float vdc_n,
...
void plecsStart(struct SimulationState *p_state)
{
    ...
    if (npc_cfg_set_ctrl_ts((float)PLECS_NPC_CONTROL_PERIOD_S) == 0U)
        initialized = false;
...
{
    const float values[]       = {(float)run_enable,
                                  v_dc_p,
                                  v_dc_n,
                                  v_out_a,
                                  v_out_b,
                                  ...
                                  output_frame[6],
                                  midpoint_kp,
                                  midpoint_current_ref,
                                  midpoint_current,
                                  midpoint_offset,
                                  midpoint_delta_filtered,
                                  midpoint_correction,
                                  midpoint_current_magnitude};
    static const char header[] = "time_s,run,vdc_p,vdc_n,va,vb,vc,ia,ib,ic,theta,ref,alpha_pwm,beta_pwm,"
