...
int npc_trace_start(const char *p_header)
{
    ...
    if (
        (p_header == NULL)
     || (GetModuleHandleExA(GET_MODULE_HANDLE_EX_FLAG_FROM_ADDRESS | GET_MODULE_HANDLE_EX_FLAG_UNCHANGED_REFCOUNT, &module_anchor, &module) == 0))
    {
        return 0;
    }
    ...
    GetSystemTime(&stamp);
    ++trace_sequence;
    written = swprintf(p_separator,
                       MAX_PATH - (size_t)(p_separator - path),
                       L"npc_trace_%04u%02u%02u_%02u%02u%02u_%03u_%lu_%u.csv",
                       (unsigned int)stamp.wYear,
                       (unsigned int)stamp.wMonth,
                       (unsigned int)stamp.wDay,
                       (unsigned int)stamp.wHour,
                       (unsigned int)stamp.wMinute,
                       (unsigned int)stamp.wSecond,
                       (unsigned int)stamp.wMilliseconds,
...
int npc_log_start(void)
{
    ...
    if (GetModuleHandleExA(GET_MODULE_HANDLE_EX_FLAG_FROM_ADDRESS | GET_MODULE_HANDLE_EX_FLAG_UNCHANGED_REFCOUNT, &module_anchor, &module) == 0)
    {
        return 0;
    }
    length = GetModuleFileNameW(module, path, MAX_PATH);
