...
REG_SHELL_VAR(SIM_TIME_S, s_state.sim_time_s, SHELL_FP32, FLT_MAX, 0.0f, NULL, SHELL_STA_NULL)
...
REG_SHELL_VAR(SIM_OFFSET, s_state.offset, SHELL_FP32, 100.0f, -100.0f, frame_bridge_parameter_changed, SHELL_STA_NULL)
...
REG_SCOPE_EX(frame_simulation, 512u, 128u, 100u, scope_sim_input, scope_sim_output, scope_sim_gain, scope_sim_offset)

REG_PERF_BASE_CNT(&s_perf_counter, 1.0e-7f)

REG_SFRA(frame_gain, 0u, 100.0e-6f, 0.05f, 10.0f, 1000.0f, NULL, NULL)
...
void plecs_perf_counter_refresh(void)
{
    ...
    if (frequency.QuadPart > 0)
    {
        ...
        counter_100ns = (whole_seconds * 10000000u) + ((remainder * 10000000u) / (uint64_t)frequency.QuadPart);
