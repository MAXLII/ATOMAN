...
{
    sogi_init(&fixture.dut, SOGI_TEST_PERIOD_S, SOGI_TEST_W0_RAD_S, SOGI_TEST_K, &fixture.input);
...
static uint8_t sogi_assert(void)
{
    ...
    (void)printf(
        "    SOGI RESULT | samples=%u amp_u=%.6f phase_u=%.3fdeg amp_qu=%.6f phase_qu=%.3fdeg err_rms=%.6f thd_u=%.6f\n",
    ...
    if (expect_true("in-phase amplitude", (fabsf(fixture.amp_u - SOGI_TEST_FUNDAMENTAL_AMP) <= SOGI_TEST_AMP_TOLERANCE) ? 1u : 0u) == 0u)
    {
        passed = 0u;
    }

    if (expect_true("in-phase angle near 0 deg", (fabsf(fixture.phase_u_rad) <= SOGI_TEST_PHASE_TOLERANCE_RAD) ? 1u : 0u) == 0u)
    {
        passed = 0u;
    }

    if (expect_true("quadrature amplitude", (fabsf(fixture.amp_qu - SOGI_TEST_FUNDAMENTAL_AMP) <= SOGI_TEST_AMP_TOLERANCE) ? 1u : 0u) == 0u)
    {
        passed = 0u;
    }

    if (expect_true("quadrature angle near -90 deg", (fabsf(fixture.phase_qu_rad + M_PI_2) <= SOGI_TEST_PHASE_TOLERANCE_RAD) ? 1u : 0u) == 0u)
    {
        passed = 0u;
    }
    ...
    if (fixture.scenario == SOGI_TEST_HARMONIC_E)
    {
        const float input_thd =
            sqrtf(0.5f * ((SOGI_TEST_H3_AMP * SOGI_TEST_H3_AMP) + (SOGI_TEST_H5_AMP * SOGI_TEST_H5_AMP)))
            / SOGI_TEST_FUNDAMENTAL_AMP;

        if (expect_true("output THD below limit and below input THD", (    (fixture.thd_u < SOGI_TEST_HARMONIC_THD_U_LIMIT) && (fixture.thd_u < input_thd)) ? 1u : 0u) == 0u)
        {
            passed = 0u;
        }

        if (expect_true("error carries the rejected harmonics", (fixture.err_rms <= SOGI_TEST_HARMONIC_ERR_RMS_LIMIT) ? 1u : 0u) == 0u)
        {
            passed = 0u;
        }
    }
    ...
    {
        if (expect_true("steady error near zero", (fixture.err_rms <= SOGI_TEST_STEADY_ERR_RMS_LIMIT) ? 1u : 0u) == 0u)
        {
            passed = 0u;
        }
    }
    ...
    if (fixture.scenario == SOGI_TEST_FREQ_UPDATE_E)
    {
        ...
        if (expect_true("center frequency set to 60 Hz", (fabsf(fixture.dut.w - SOGI_TEST_W1_RAD_S) <= 1.0e-3f) ? 1u : 0u) == 0u)
        {
            passed = 0u;
        }
    }
...
TESTBENCH_CASE(sogi, steady_50hz, steady_init, sogi_before_dut, sogi_after_dut)

TESTBENCH_CASE(sogi, frequency_update_60hz, frequency_update_init, sogi_before_dut, sogi_after_dut)

TESTBENCH_CASE(sogi, harmonic_rejection, harmonic_init, sogi_before_dut, sogi_after_dut)
