                         ...
                         uint8_t include_dc_point)
{
    ...
    {
        status = sfra_core_set_sweep_range(&fixture.sfra, PI_TEST_SFRA_START_HZ, PI_TEST_SFRA_END_HZ);
    }
...
{
    sfra_prepare(PI_TEST_SFRA_CLOSED_LOOP_E, "build/pi_sfra_closed_loop_bode.csv", 1u);
}

static void sfra_open_loop_init(void)
{
    sfra_prepare(PI_TEST_SFRA_OPEN_LOOP_E, "build/pi_sfra_open_loop_bode.csv", 0u);
}
...
static void plant_update(void)
{
    fixture.current_a += (fixture.voltage_v / PI_TEST_INDUCTANCE_H) * PI_TEST_CONTROL_PERIOD_S;
...
static void sfra_record(double time_s)
{
    ...
    if (fixture.scenario == PI_TEST_SFRA_CLOSED_LOOP_E)
    {
        ...
        fabsf(fixture.sfra.output.current_freq_hz - PI_TEST_BANDWIDTH_HZ) < fabsf(fixture.crossover_freq_hz - PI_TEST_BANDWIDTH_HZ))
    {
...
static uint8_t step_assert(void)
{
    ...
    if (expect_true("final error within tolerance", (final_error_a <= PI_TEST_STABLE_ERROR_A) ? 1u : 0u) == 0u)
    {
        passed = 0u;
    }
...
{
    uint8_t passed                  = 1u;
    const float final_freq_error_hz = fabsf(fixture.sfra.output.current_freq_hz - PI_TEST_SFRA_END_HZ);
    ...
    if (expect_true("all SFRA points recorded", (fixture.sfra_recorded_point_count == PI_TEST_SFRA_POINT_COUNT) ? 1u : 0u) == 0u)
    {
        passed = 0u;
    }

    if (expect_true("20 kHz endpoint reached", (final_freq_error_hz <= 1.0f) ? 1u : 0u) == 0u)
    {
        passed = 0u;
    }
    ...
    if (fixture.scenario == PI_TEST_SFRA_OPEN_LOOP_E)
    {
        ...
        if (expect_true("10 kHz crossover magnitude", (    (fixture.crossover_magnitude >= 0.9f) && (fixture.crossover_magnitude <= 1.1f)) ? 1u : 0u) == 0u)
        {
            passed = 0u;
        }
    }
...
TESTBENCH_CASE(pi, current_reference_step, step_init, step_before_dut, step_after_dut)

TESTBENCH_CASE(pi, sfra_closed_loop_sweep, sfra_closed_loop_init, sfra_feedback_loop_before_dut,
               sfra_closed_loop_after_dut)

TESTBENCH_CASE(pi, sfra_open_loop_sweep, sfra_open_loop_init, sfra_open_loop_before_dut, sfra_open_loop_after_dut)
