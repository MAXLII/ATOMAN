...
static void test_command_handler(void *p_frame, DEC_MY_PRINTF)
{
    ...
    if ((p_pack->len > 0u) && (p_pack->p_data != NULL) && (p_pack->len <= sizeof(g_last_data)))
    {
...
static void test_tx_by_dma_cb(char *ptr, int len)
{
    ...
    if ((ptr != NULL) && (copy_len <= sizeof(g_tx_bytes)))
    {
...
static uint32_t test_build_msg(uint8_t seq, uint8_t dst, uint8_t d_dst,
                               uint8_t src, uint8_t d_src,
                               uint8_t cmd_set, uint8_t cmd_word,
                               uint8_t codec, uint8_t cmd, uint8_t ack)
{
    return (uint32_t)seq |
           ((uint32_t)dst << 3u) |
           ((uint32_t)d_dst << 7u) |
           ((uint32_t)src << 10u) |
           ((uint32_t)d_src << 14u) |
           ((uint32_t)cmd_set << 17u) |
           ((uint32_t)cmd_word << 21u) |
           ((uint32_t)codec << 27u) |
           ((uint32_t)cmd << 30u) |
           ((uint32_t)ack << 31u);
}

static uint16_t test_build_frame(uint8_t *p_out, uint32_t msg, uint8_t cmd,
...
static void test_codec_dict(void)
{
    ...
    CHECK(codec_dict_encode(input_len, (const uint8_t *)input, NULL, encoded, 300u) == 0, "DICT rejects NULL length pointer");
...
static void test_codec_zero(void)
{
    ...
    /* Small-value dense data compresses well: 16 zero bytes -> 8 bytes. */
    {
        ...
        CHECK(out_len == 16u && memcmp(back, zeros, 16u) == 0, "ZERO zeros roundtrip");
    }
     ...
     * the codec still roundtrips for any input. */
    {
        ...
        CHECK(out_len == 16u && memcmp(back, high, 16u) == 0, "ZERO high nibble roundtrip");
    }
...
static void test_tx_compression(void)
{
    ...
    msg = (uint32_t)g_tx_bytes[1] |
          ((uint32_t)g_tx_bytes[2] << 8u) |
          ((uint32_t)g_tx_bytes[3] << 16u) |
...
static void test_tx_codec_select(void)
{
    ...
    response_crc = (uint32_t)g_tx_bytes[10] |
                   ((uint32_t)g_tx_bytes[11] << 8u) |
                   ((uint32_t)g_tx_bytes[12] << 16u) |
