...
static void check_ti_balanced(const svpwm_3level_t *p_svpwm)
{
    ...
    const double phases[3] = {alpha,
                              (-0.5 * alpha) + (sqrt(3.0) * beta * 0.5),
...
static void verify_sample(svpwm_3level_t *p_svpwm)
{
    ...
    ++sample_count;
    {
        ...
        if (    (p_svpwm->input.v_dc_p == p_svpwm->input.v_dc_n)
             && (gain == 1.0)
             && (p_svpwm->cfg.average_balance == false))
        {
    ...
    for (uint32_t mode = 0u; mode < 2u; ++mode) /* Exercise both sector selection and averaged balancing. */
    {
        for (size_t b = 0u; b < (sizeof(buses) / sizeof(buses[0])); ++b) /* Bus case index. */
        {
            ...
            for (uint32_t angle = 0u; angle < 720u; ++angle) /* Half-degree angular step. */
            {
                ...
                double limit = ((double)buses[b][0] + buses[b][1]) / (fmax(a, fmax(vb, vc)) - fmin(a, fmin(vb, vc))); /* Physical hexagon radius. */
...
static void test_lifecycle(void)
{
    ...
    verify_sample(&instance);
    instance.input =
        (svpwm_3level_input_t){.v_alpha = 0.5f * FLT_MAX, .v_beta = 0.0f, .v_dc_p = FLT_MAX, .v_dc_n = FLT_MAX};
    verify_sample(&instance);
}
