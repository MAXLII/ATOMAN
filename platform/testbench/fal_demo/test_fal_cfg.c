...
static fal_result_t finish(uint8_t device)
{
    ...
    while (    (fal_is_busy(&g_demo_fal[device]) == 1u)
            && (steps < 100u))
    {
...
int main(void)
{
    ...
    for (uint8_t device = 0u; device < DEMO_FAL_DEVICE_COUNT; device++)
    {
        ...
        (void)printf("INPUT device=%u page=%lu block=%lu test_zone=%u\n",
                     (unsigned)device,
                     (unsigned long)page,
                     (unsigned long)p_device->erase_block_size,
                     (unsigned)test_zone);
        ...
        for (uint32_t i = 0u; i < page; i++)
        {
            source[i] = (uint8_t)(i ^ (i >> 8u));
        }
        check("direct write accepted",
              FAL_RESULT_IN_PROGRESS,
              fal_write(&g_demo_fal[device], test_zone, 0u, page, source));
        check("direct write succeeds", 0, finish(device));
        check("direct read accepted",
              FAL_RESULT_IN_PROGRESS,
