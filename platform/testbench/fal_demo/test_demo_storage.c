...
static demo_storage_snapshot_t drain(void)
{
    ...
    do
    {
        ...
        step++;
    } while (    (snapshot.busy != 0u)
              && (step < 4000u));
    check("transaction completed", 0, snapshot.busy);
    (void)printf("OUTPUT steps=%lu result=%ld RAM=%lu,%lu,%lu seq=%lu,%lu\n",
                 (unsigned long)step,
                 (long)snapshot.result,
                 (unsigned long)snapshot.params.input_mv,
                 (unsigned long)snapshot.params.gain_milli,
                 (unsigned long)snapshot.params.threshold_mv,
                 (unsigned long)snapshot.devices[0].sequence,
                 (unsigned long)snapshot.devices[1].sequence);
    return snapshot;
}
static void reset_case(const char *name)
{
    ...
    fake_reset();
    (void)fal_runtime_init(&g_demo_fal_runtime);
    demo_storage_service_init();
    (void)drain();
...
static void transactions(void)
{
    ...
    for (uint8_t d = 0u; d < 2u; d++)
    {
        ...
        s = drain();
        check("save result", 0, s.result);
        check("initial sequence", 1, s.devices[d].sequence);
        configure(4567u);
        s = command(d, DEMO_STORAGE_LOAD);
        check("load restores data pool", 1234u + d, s.params.input_mv);
        configure(6789u);
        s = command(d, DEMO_STORAGE_SAVE);
        check("A/B next sequence", 2, s.devices[d].sequence);
        s = command(d, DEMO_STORAGE_TEST);
        check("cross-block test", 0, s.result);
        s = command(d, DEMO_STORAGE_LOAD);
        check("scratch preserves parameters", 6789, s.params.input_mv);

        for (uint32_t i = 1u; i <= 6u; i++)
        {
            configure(i);
            s = command(d, DEMO_STORAGE_LOG);
            check("log append result", 0, s.result);
            check("ring sequence", i, s.devices[d].log_sequence);
        }
        s = command(d, DEMO_STORAGE_SCAN);
        check("log rescan latest input", 6, s.devices[d].last_log.input_mv);
        {
            uint32_t count = fake_writes(); /* Defaults only changes RAM. */
            s              = command(d, DEMO_STORAGE_DEFAULTS);
            check("defaults input", 1000, s.params.input_mv);
    ...
    demo_storage_service_init();
    {
        demo_storage_snapshot_t s = drain();
        check("reboot selects NOR parameters", 6789, s.params.input_mv);
    }
...
static void failures(void)
{
    ...
    configure(1111u);
    (void)command(0u, DEMO_STORAGE_SAVE);
    configure(2222u);
    fake_program_fault(0u, 0x1F9000u + 256u);
    {
        demo_storage_snapshot_t s = command(0u, DEMO_STORAGE_SAVE);
        check("commit error propagated", FAL_RESULT_DRIVER_ERROR, s.result);
    }
    fake_program_fault(0u, UINT32_MAX);
    {
        demo_storage_snapshot_t s = command(0u, DEMO_STORAGE_LOAD);
        check("uncommitted B ignored", 1111, s.params.input_mv);
    }
    configure(3333u);
    (void)command(0u, DEMO_STORAGE_SAVE);
    ...
    {
        demo_storage_snapshot_t s = command(0u, DEMO_STORAGE_LOAD);
        check("CRC bad B falls back A", 1111, s.params.input_mv);
    }
    fake_read_fault(0u, 0x1F8000u);
    {
        uint32_t count            = fake_writes();
        demo_storage_snapshot_t s = command(0u, DEMO_STORAGE_SAVE);
        check("unreadable scan blocks SAVE", -23, s.result);
        check("no destructive retry", count, fake_writes());
    }
    fake_read_fault(0u, UINT32_MAX);
    reset_case("NOR absent does not prevent NAND use");
    fake_present(0u, 0u);
    {
        demo_storage_snapshot_t s = command(0u, DEMO_STORAGE_SCAN);
        check("absent device result", -22, s.result);
    }
    configure(4444u);
    {
        demo_storage_snapshot_t s = command(1u, DEMO_STORAGE_SAVE);
        check("independent NAND save", 0, s.result);
    }
}
...
static void alignment(void)
{
    ...
    device.p_zones    = zones;
    device.zone_count = 2u;
...
static void integrity(void)
{
    ...
    {
        (void)memcpy(data, original, 512u);
        (void)memcpy(ecc, saved, 7u);
        flip_bit(data, ecc, bit);
    ...
    for (uint32_t trial = 0u; trial < 100u; trial++)
    {
        ...
        {
            flip_bit(data, ecc, 52u + start + i);
        }
        result = bch4_correct(data, ecc);
        check(    "five-bit corruption rejected by BCH/CRC",
                  1,
                  (result < 0)
                   || (flash_integrity_crc32(data, 512u) != flash_integrity_crc32(original, 512u)));
    }
