...
static uint32_t capacity(uint8_t d)
{
    return d == 0u ? 2097152u : 134217728u;
}
static uint32_t block_size(uint8_t d)
{
    return d == 0u ? 4096u : 131072u;
}
static uint32_t base(uint8_t d)
{
    return capacity(d) - 8u * block_size(d);
}
void fake_reset(void)
{
    ...
    for (uint8_t d = 0u; d < 2u; d++)
    {
        ...
        busy[d]          = 0u;
        errors[d]        = 0;
        read_fault[d]    = UINT32_MAX;
        program_fault[d] = UINT32_MAX;
    }
    writes        = 0u;
    prefix_writes = 0u;
}
void fake_present(uint8_t d, uint8_t value)
{
    present[d] = value;
}
void fake_read_fault(uint8_t d, uint32_t address)
{
    read_fault[d] = address;
}
void fake_program_fault(uint8_t d, uint32_t address)
{
    program_fault[d] = address;
}
void fake_flip(uint8_t d, uint32_t address)
{
    assert(address >= base(d));
    memory[d][address - base(d)] ^= 1u;
}
...
{
    return prefix_writes;
}
BSP_FLASH_RESULT_E bsp_flash_init(uint8_t d)
{
    busy[d] = 0u;
    errors[d] = present[d] == 1u ? 0 : -1;
    return errors[d] == 0 ? BSP_FLASH_SUCCESS : BSP_FLASH_IO_ERROR;
}
...
void bsp_flash_geometry_get(uint8_t d,
                            uint32_t *cap,
                            uint32_t *page,
                            uint32_t *block,
                            uint32_t *unit)
...
void bsp_flash_health_get(uint8_t d, bsp_flash_health_t *health)
{
    ...
    health->id = d == 0u ? 0xC84015u : 0xC8F1801Du;
    health->error = errors[d];
}
BSP_FLASH_RESULT_E bsp_flash_read(uint8_t d,
                                  uint32_t address,
                                  uint32_t length,
                                  uint8_t *data)
{
    if (address == read_fault[d])
    {
        errors[d] = -5;
        return BSP_FLASH_IO_ERROR;
    }
    assert(    address >= base(d)
            && length <= capacity(d) - address);
    (void)memcpy(data, &memory[d][address - base(d)], length);
    busy[d] = 1u;
    return BSP_FLASH_SUCCESS;
}
BSP_FLASH_RESULT_E bsp_flash_program(uint8_t d,
                                     uint32_t address,
                                     uint32_t length,
                                     const uint8_t *data)
{
    ...
    {
        prefix_writes++;
        return BSP_FLASH_INVALID_ARGUMENT;
    }

    if (address == program_fault[d])
    {
        errors[d] = -6;
        return BSP_FLASH_IO_ERROR;
    }
...
BSP_FLASH_RESULT_E bsp_flash_erase(uint8_t d, uint32_t address, uint32_t length)
{
    ...
    assert(    address % block_size(d) == 0u
            && length == block_size(d));
    assert(length <= capacity(d) - address);
