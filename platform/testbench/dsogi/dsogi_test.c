...
static void positive_init(void)
{
    select_case(0u);
}
/** @brief Pure negative sequence. */
static void negative_init(void)
{
    select_case(1u);
}
/** @brief Mixed positive and negative sequences. */
static void mixed_init(void)
{
    select_case(2u);
}
/** @brief Balanced input to the DSOGI/PLL chain. */
static void pll_init_case(void)
{
    select_case(3u);
}
...
{
    const dsogi_cfg_t cfg = {.ts = (float)TEST_TS, .k = 1.41421356237f, .omega_min = 250.0f, .omega_max = 400.0f}; /* 40..63 Hz tuning envelope. */
    alpha = 0.0f;
    beta  = 0.0f;
    omega = (float)(2.0 * TEST_PI * 50.0);
    expect(dsogi_init(&filter, &cfg, &alpha, &beta, &omega));
    expect(pll_init(&loop,
                    (float)TEST_TS,
                    omega,
                    400.0f,
                    250.0f,
                    ...
                    -64.0f,
                    0.0f,
                    &filter.output.alpha_pos,
                    &filter.output.beta_pos));
...
static TESTBENCH_CASE_STATE_E after_dut(double time_s)
{
    ...
    {
        (void)fprintf(p_csv,
                      "%.6f,%.6g,%.6g,%.6g,%.6g,%.6g,%.6g,%.6g,%.6g,%d\n",
                      time_s,
                      (double)alpha,
                      (double)beta,
                      (double)filter.output.alpha_pos,
                      (double)filter.output.beta_pos,
                      (double)filter.output.alpha_neg,
                      (double)filter.output.beta_neg,
                      (double)loop.output.omega / (2.0 * TEST_PI),
                      (double)loop.output.theta,
                      (int)result);
    }
    ++tick;
