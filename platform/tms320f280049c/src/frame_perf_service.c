...
#define FRAME_PERF_DICT_NAME_LENGTH_MAX          (PERF_OPT_MAX_PAYLOAD_SIZE - FRAME_PERF_DICT_ITEM_FIXED_SIZE)
...
static uint16_t frame_perf_record_count(uint8_t type_filter)
{
    ...
    while (p_item != NULL)
    {
        section_perf_record_t *p_record = perf_record_from_item(p_item); /* Perf record recovered through the shared adapter. */
...
static void frame_perf_transfer_end(uint8_t status)
{
    ...
    {
        wire_u32_le_write(&frame_perf_context.payload[FRAME_PERF_DICT_END_SEQUENCE_OFFSET],
                          frame_perf_context.sequence);
        wire_u16_le_write(&frame_perf_context.payload[FRAME_PERF_DICT_END_RECORD_COUNT_OFFSET],
                          frame_perf_context.record_index);
        frame_perf_context.payload[FRAME_PERF_DICT_END_STATUS_OFFSET]   = wire_octet_get(status);
        frame_perf_context.payload[FRAME_PERF_DICT_END_RESERVED_OFFSET] = 0u;
        wire_u32_le_write(&frame_perf_context.payload[FRAME_PERF_DICT_END_VERSION_OFFSET],
                          frame_perf_context.dict_version);
        frame_perf_report(PERF_OPT_CMD_DICT_END, frame_perf_context.payload, FRAME_PERF_DICT_END_SIZE);
    }
    else if (frame_perf_context.pull_type == PERF_OPT_PULL_SAMPLE)
    {
        wire_u32_le_write(&frame_perf_context.payload[FRAME_PERF_SAMPLE_END_SEQUENCE_OFFSET],
                          frame_perf_context.sequence);
        wire_u16_le_write(&frame_perf_context.payload[FRAME_PERF_SAMPLE_END_RECORD_COUNT_OFFSET],
                          frame_perf_context.record_index);
        frame_perf_context.payload[FRAME_PERF_SAMPLE_END_STATUS_OFFSET]   = wire_octet_get(status);
        frame_perf_context.payload[FRAME_PERF_SAMPLE_END_RESERVED_OFFSET] = 0u;
        frame_perf_report(PERF_OPT_CMD_SAMPLE_END, frame_perf_context.payload, FRAME_PERF_SAMPLE_END_SIZE);
    }
    else
    {
...
static void frame_perf_dictionary_poll(void)
{
    ...
    name_length = frame_perf_name_copy(&frame_perf_context.payload[FRAME_PERF_DICT_ITEM_NAME_OFFSET], p_record->p_name);

    wire_u32_le_write(&frame_perf_context.payload[FRAME_PERF_DICT_ITEM_SEQUENCE_OFFSET], frame_perf_context.sequence);
    wire_u16_le_write(&frame_perf_context.payload[FRAME_PERF_DICT_ITEM_INDEX_OFFSET], frame_perf_context.record_index);
    wire_u16_le_write(&frame_perf_context.payload[FRAME_PERF_DICT_ITEM_RECORD_COUNT_OFFSET],
                      frame_perf_context.record_count);
    wire_u16_le_write(&frame_perf_context.payload[FRAME_PERF_DICT_ITEM_RECORD_ID_OFFSET], p_record->record_id);
    frame_perf_context.payload[FRAME_PERF_DICT_ITEM_RECORD_TYPE_OFFSET] = wire_octet_get(protocol_type);
    frame_perf_context.payload[FRAME_PERF_DICT_ITEM_NAME_LENGTH_OFFSET] = wire_octet_get(name_length);

    payload_length = (uint16_t)(FRAME_PERF_DICT_ITEM_FIXED_SIZE + name_length);
    frame_perf_report(PERF_OPT_CMD_DICT_ITEM_REPORT, frame_perf_context.payload, payload_length);
                                             ...
                                             wire_octet_t *p_destination)
{
    ...
    wire_u16_le_write(&p_destination[FRAME_PERF_SAMPLE_ITEM_RECORD_ID_OFFSET], p_record->record_id);
    wire_u32_le_write(&p_destination[FRAME_PERF_SAMPLE_ITEM_TIME_US_OFFSET], perf_count_to_us(p_record->time));
    wire_u32_le_write(&p_destination[FRAME_PERF_SAMPLE_ITEM_MAX_TIME_US_OFFSET], perf_count_to_us(p_record->max_time));

    switch (p_record->record_type)
    {
    case SECTION_PERF_RECORD_TASK:
        wire_u32_le_write(&p_destination[FRAME_PERF_SAMPLE_TASK_PERIOD_US_OFFSET], perf_task_period_us_get(p_record));
        wire_f32_le_write(&p_destination[FRAME_PERF_SAMPLE_TASK_LOAD_OFFSET], p_record->load * 100.0f);
        wire_f32_le_write(&p_destination[FRAME_PERF_SAMPLE_TASK_PEAK_OFFSET], p_record->load_max * 100.0f);
        item_size = FRAME_PERF_SAMPLE_TASK_ITEM_SIZE;
        break;

    case SECTION_PERF_RECORD_INTERRUPT:
        wire_f32_le_write(&p_destination[FRAME_PERF_SAMPLE_INTERRUPT_LOAD_OFFSET], p_record->load * 100.0f);
        wire_f32_le_write(&p_destination[FRAME_PERF_SAMPLE_INTERRUPT_PEAK_OFFSET], p_record->load_max * 100.0f);
...
static void frame_perf_sample_poll(void)
{
    ...
    item_size = frame_perf_sample_item_write(p_record, &frame_perf_context.payload[FRAME_PERF_SAMPLE_ITEM_OFFSET]);
    ...
    wire_u32_le_write(&frame_perf_context.payload[FRAME_PERF_SAMPLE_BATCH_SEQUENCE_OFFSET],
                      frame_perf_context.sequence);
    wire_u16_le_write(&frame_perf_context.payload[FRAME_PERF_SAMPLE_BATCH_RECORD_COUNT_OFFSET],
                      frame_perf_context.record_count);
    wire_u16_le_write(&frame_perf_context.payload[FRAME_PERF_SAMPLE_BATCH_ITEM_COUNT_OFFSET], 1u);

    payload_length = (uint16_t)(FRAME_PERF_SAMPLE_BATCH_HEADER_SIZE + item_size);
    frame_perf_report(PERF_OPT_CMD_SAMPLE_BATCH_REPORT, frame_perf_context.payload, payload_length);
                                      ...
                                      section_link_tx_func_t *p_output)
{
    ...
    wire_u16_le_write(&payload[FRAME_PERF_INFO_PROTOCOL_VERSION_OFFSET], FRAME_PERF_PROTOCOL_VERSION);
    wire_u16_le_write(&payload[FRAME_PERF_INFO_RECORD_COUNT_OFFSET], frame_perf_record_count(PERF_OPT_TYPE_ALL));
    wire_f32_le_write(&payload[FRAME_PERF_INFO_UNIT_US_OFFSET], perf_count_unit_us_get());
    wire_u32_le_write(&payload[FRAME_PERF_INFO_COUNT_PER_TICK_OFFSET], perf_cnt_per_sys_tick_get());
    wire_u32_le_write(&payload[FRAME_PERF_INFO_CPU_WINDOW_MS_OFFSET], PERF_CPU_LOAD_PERIOD_MS);
    payload[FRAME_PERF_INFO_FLAGS_OFFSET] = wire_octet_get(FRAME_PERF_INFO_FLAGS);

    frame_perf_reply(p_request, p_output, PERF_OPT_CMD_INFO_QUERY, payload, FRAME_PERF_INFO_ACK_SIZE);
}
                                         ...
                                         section_link_tx_func_t *p_output)
{
    ...
    wire_f32_le_write(&payload[FRAME_PERF_SUMMARY_TASK_LOAD_OFFSET], perf_task_metric_get() * 100.0f);
    wire_f32_le_write(&payload[FRAME_PERF_SUMMARY_TASK_PEAK_OFFSET], perf_task_metric_max_get() * 100.0f);
    wire_f32_le_write(&payload[FRAME_PERF_SUMMARY_INTERRUPT_LOAD_OFFSET], perf_interrupt_metric_get() * 100.0f);
    wire_f32_le_write(&payload[FRAME_PERF_SUMMARY_INTERRUPT_PEAK_OFFSET], perf_interrupt_metric_max_get() * 100.0f);

    frame_perf_reply(p_request, p_output, PERF_OPT_CMD_SUMMARY_QUERY, payload, FRAME_PERF_SUMMARY_ACK_SIZE);
}
                                      ...
                                      section_link_tx_func_t *p_output)
{
    ...
    frame_perf_reply(p_request, p_output, PERF_OPT_CMD_RESET_PEAK, payload, FRAME_PERF_RESET_ACK_SIZE);
}
                                      ...
                                      section_link_tx_func_t *p_output)
{
    ...
    if (frame_perf_request_payload_is_valid(p_request, FRAME_PERF_DICT_QUERY_SIZE) == 1u)
    {
        type_filter        = wire_octet_get(p_request->p_data[FRAME_PERF_DICT_QUERY_FILTER_OFFSET]);
        known_dict_version = wire_u32_le_read(&p_request->p_data[FRAME_PERF_DICT_QUERY_VERSION_OFFSET]);
        (void)known_dict_version;

        accepted = frame_perf_transfer_start(p_request, p_output, type_filter, PERF_OPT_PULL_DICT, &reject_reason);
    }
    ...
    wire_u32_le_write(&payload[FRAME_PERF_DICT_ACK_VERSION_OFFSET], current_dict_version);
    payload[FRAME_PERF_DICT_ACK_REJECT_REASON_OFFSET] = wire_octet_get(reject_reason);

    if (accepted == 1u)
    {
        wire_u16_le_write(&payload[FRAME_PERF_DICT_ACK_RECORD_COUNT_OFFSET], frame_perf_context.record_count);
        wire_u32_le_write(&payload[FRAME_PERF_DICT_ACK_SEQUENCE_OFFSET], frame_perf_context.sequence);
    }

    frame_perf_reply(p_request, p_output, PERF_OPT_CMD_DICT_QUERY, payload, FRAME_PERF_DICT_ACK_SIZE);
}
                                        ...
                                        section_link_tx_func_t *p_output)
{
    ...
    if (frame_perf_request_payload_is_valid(p_request, FRAME_PERF_SAMPLE_QUERY_SIZE) == 1u)
    {
        type_filter        = wire_octet_get(p_request->p_data[FRAME_PERF_SAMPLE_QUERY_FILTER_OFFSET]);
        query_flags        = wire_octet_get(p_request->p_data[FRAME_PERF_SAMPLE_QUERY_FLAGS_OFFSET]);
        query_dict_version = wire_u32_le_read(&p_request->p_data[FRAME_PERF_SAMPLE_QUERY_VERSION_OFFSET]);
        (void)query_flags;
        ...
        {
            accepted =
                frame_perf_transfer_start(p_request, p_output, type_filter, PERF_OPT_PULL_SAMPLE, &reject_reason);
        }
    }
    ...
    wire_u32_le_write(&payload[FRAME_PERF_SAMPLE_ACK_VERSION_OFFSET], current_dict_version);
    payload[FRAME_PERF_SAMPLE_ACK_REJECT_REASON_OFFSET] = wire_octet_get(reject_reason);

    if (accepted == 1u)
    {
        wire_u16_le_write(&payload[FRAME_PERF_SAMPLE_ACK_RECORD_COUNT_OFFSET], frame_perf_context.record_count);
        wire_u32_le_write(&payload[FRAME_PERF_SAMPLE_ACK_SEQUENCE_OFFSET], frame_perf_context.sequence);
    }

    frame_perf_reply(p_request, p_output, PERF_OPT_CMD_SAMPLE_QUERY, payload, FRAME_PERF_SAMPLE_ACK_SIZE);
}
                                          ...
                                          section_link_tx_func_t *p_output)
{
    ...
    if (frame_perf_request_payload_is_valid(p_request, FRAME_PERF_CONTROL_QUERY_SIZE) == 1u)
    {
        enable = wire_octet_get(p_request->p_data[FRAME_PERF_CONTROL_ENABLE_OFFSET]);
    }
    ...
    frame_perf_reply(p_request, p_output, PERF_OPT_CMD_REPORT_CONTROL, payload, FRAME_PERF_CONTROL_ACK_SIZE);
}
