...
static double learning_u[2][36][2] = {{{0}}};
static double learning_v[2][36][2] = {{{0}}};
static void candidate_learning(npc_ctrl_t *p_control, const npc_ctrl_sample_t *p_sample, int delay, int tick)
{
    ...
    for (int harmonic = 2; harmonic <= 35; ++harmonic)
    {
        ...
        if ((tick + 1) % 100 == 0)
        {
            ...
            for (int axis = 0; axis < 2; ++axis)
            {
                ...
                learning_u[axis][harmonic][0] = .999 * learning_u[axis][harmonic][0] - .1*(real_comp*re-imaginary_comp*im);
                learning_u[axis][harmonic][1] = .999 * learning_u[axis][harmonic][1] - .1*(real_comp*im+imaginary_comp*re);
                double scale = fmin(1.0,50.0/fmax(hypot(learning_u[axis][harmonic][0],learning_u[axis][harmonic][1]),1e-6));
    ...
    double phases[3] = {command[0],-.5*command[0]+M_SQRT3_2*command[1],-.5*command[0]-M_SQRT3_2*command[1]};
