...
static double resonance_v_history[2][2] = {{0}};
static double resonance_y_history[2][2] = {{0}};
...
static void candidate_average_balance(svpwm_3level_t *p_mod, int mode)
{
    ...
    if ((mode >= 72) && (fabs(balance_gain)>1e-6))
    {
        balance_integral += .0002 * (balance_delta_filtered+
            10.0*(offset-base_offset-correction)/(balance_gain*M_2PI*M_2PI));
    }
