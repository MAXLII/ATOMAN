...
{
    const svpwm_3level_phase_output_t *phases[3] = {
    ...
    for (int phase=0;phase<3;++phase)
    {
        ...
        assert(positive>=0.0f && positive<=1.0f && negative>=0.0f && negative<=1.0f);
        ...
        reconstructed[phase]=(double)phases[phase]->duty_p*p_mod->input.v_dc_p-
...
int main(int argc, char **p_argv)
{
    ...
    svpwm_3level_cfg_t mod_cfg = {
        .v_dc_half_min = 20.0f, .midpoint_kp = mode == 0 ? 0.0f : 1.8849556f
    };
    if ((mode == 80) || (mode == 82))
    {
    ...
    if ((mode > 16) && (mode != 80) && (mode != 82) && /* Production modes retain the actual C configuration. */
    ...
    if ((mode > 16) && (mode != 80) && (mode != 82) && /* Filter-frequency sweeps are offline only. */
    ...
    fprintf(p_file, "time,delta,ref,estimate,actual,voltage_peak,va,vb,vc,ia,ib,ic,vd_pos,vq_pos,vd_neg,vq_neg,current_limited,voltage_limited\n");
    ...
    for (int harmonic = 0; harmonic < 6; ++harmonic)
    {
        ...
        {
            (void)pr_init(&harmonics[harmonic][axis], 0.0f, 5.0f, warped, M_2PI,
                          cfg.ts, 40.0f, -40.0f, &harmonic_reference, &harmonic_residual[axis]);
        }
    }
    for (int k = 0; k < 60000; ++k)
    {
        int stage = (k >= 20000 && k < 40000) ? scenario : 0;
        ...
        for (int p = 0; p < 3; ++p)
        {
            ...
            for (int state = 0; state < 10; ++state)
            {
                terminal_voltage += (inverse_clarke[p][0] * voltage_output[0][0][state] +
                                     inverse_clarke[p][1] * voltage_output[0][1][state]) * x[state];
            }
        ...
        mod.input = (svpwm_3level_input_t){
            .v_alpha = control.output.v_alpha, .v_beta = control.output.v_beta, .v_dc_p = (float)vp, .v_dc_n = (float)vn
        };
        double currents[3] = {0};
        for (int p = 0; p < 3; ++p)
        {
            ...
            (mode < 30))   /* Later cases isolate dead-time compensation without resonant controllers. */
        {
            ...
            harmonic_residual[0] = control.inter.v_ab[0] - control.inter.voltage.output.alpha_pos -
                                   control.inter.voltage.output.alpha_neg;
            harmonic_residual[1] = control.inter.v_ab[1] - control.inter.voltage.output.beta_pos -
                                   control.inter.voltage.output.beta_neg;
            for (int harmonic = 0; harmonic < 6; ++harmonic)
            {
                for (int axis = 0; axis < 2; ++axis)
                {
            ...
            (mode == 5))   /* Historical bias + common-mode slew candidate. */
        {
            ...
            for (int phase = 0; phase < 3; ++phase)
            {
                residual_filtered[phase] += 0.003762841 * (sample.i_l[phase] - control.output.i_fundamental[phase] - residual_filtered[phase]);
            ...
            (mode == 34)) /* Repeat with the production damping filter and a smaller slew. */
        {
            ...
            for (int phase = 0; phase < 3; ++phase)
            {
                phase_v[phase] = inverse_clarke[phase][0] * mod.input.v_alpha + inverse_clarke[phase][1] * mod.input.v_beta;
        ...
        if (delay_periods == 0) /* The normal runner keeps the requested one-period delay. */
        {
            ...
            if (((mode >= 30) && (mode <= 33)) ||
                (((mode >= 73 && mode < 80) || mode == 81) && (balance_current_filtered > 200.0)))
            {
                for (int p = 0; p < 3; ++p)
                {
        ...
        if (((mode == 80) || (mode == 82)) && (delay_periods == 0))
        {
        ...
        for (int sub = 0; sub < SUBSTEPS; ++sub)
        {
            ...
            for (int p = 0; p < 3; ++p)
            {
                ...
                for (int q = 0; q<4; ++q)
                {
                    ...
                    gate[q] = req[q] && age[p][q]>dead_time;
                }
                ...
                for (int phase = 0; phase < 3; ++phase)
                {
                    ...
                    for (int state = 0; state < 10; ++state)
                    {
                        terminal_voltage += (inverse_clarke[phase][0] * voltage_output[0][0][state] +
                                             inverse_clarke[phase][1] * voltage_output[0][1][state]) * x[state];
                    }
        ...
        if ((scenario == 3) && (mode != 82)) /* Historical snapshot bleeders; absent from the live model. */
        {
        ...
        const svpwm_3level_phase_output_t phases[3] = {
            mod.output.phase_a, mod.output.phase_b, mod.output.phase_c
        };
        for (int p = 0; p < 3; ++p)
        {
            duty[p][0] = phases[p].duty_p;
            duty[p][1] = phases[p].duty_n;
            if (((mode >= 13) && (mode < 16)) || /* Historical differential-damping cases. */
                ((mode >= 30) && (mode <= 33)) ||
                (((mode >= 73 && mode < 80) || mode == 81) && (balance_current_filtered > 200.0))) /* Compensate only when fundamental current dominates ripple. */
            {
                double correction = dead_time / .0002 * fmax(-1.0, fmin(1.0, currents[p] / 100.0));
                if (duty[p][0] > 0.0)
                {
        ...
        {
            fprintf(p_file, "%.5f,%.6f,%.6f,%.6f,%.6f,%.6f", t, delta, mod.output.midpoint_current_ref, mod.output.midpoint_current, charge / .0002, hypot(x[2], x[3]));
            for (int phase = 0; phase < 3; ++phase)
            {
