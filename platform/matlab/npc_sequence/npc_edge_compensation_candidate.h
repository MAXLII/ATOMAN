...
{
    const svpwm_3level_phase_output_t *p_phase[3] = {&p_mod->output.phase_a,
    ...
    {
        average[phase] = p_phase[phase]->duty_p * p_mod->input.v_dc_p - p_phase[phase]->duty_n * p_mod->input.v_dc_n;
    ...
    for (uint32_t index = 1u; index < 3u; ++index)
    {
        ...
        while (    (position > 0u)
                && (event[order[position - 1u]] > event[item]))
        {
