                                             ...
                                             const uint8_t *p_data)
{
    ...
    while (source_offset < length)
    {
        ...
        (void)memcpy(program_data, (const void *)(uintptr_t)program_address, BSP_EFM_FLASH_PROGRAM_SIZE);
        (void)memcpy(&program_data[byte_offset], &p_data[source_offset], copy_length);

        primask = __get_PRIMASK();
        __disable_irq();
        result = EFM_ProgramReadBack(program_address, program_data, BSP_EFM_FLASH_PROGRAM_SIZE);
