...
_Static_assert(HC32F334_IAP_BASE == HC32F334_BOOT_SIZE, "IAP base must immediately follow the Bootloader partition");
...
static fal_result_t efm_init(void *p_context)
{
    ...
    return (BSP_EFM_FLASH_RESULT_SUCCESS == result) ? FAL_RESULT_SUCCESS : FAL_RESULT_DRIVER_ERROR;
}

static fal_device_state_t efm_state_get(void *p_context)
{
    (void)p_context;
    return (BSP_EFM_FLASH_STATE_READY == bsp_efm_flash_state_get()) ? FAL_DEVICE_STATE_READY : FAL_DEVICE_STATE_ERROR;
}

static fal_result_t efm_read(void *p_context,
                             uint32_t address,
                             uint32_t length,
                             uint8_t *p_data)
{
    (void)p_context;
    return (BSP_EFM_FLASH_RESULT_SUCCESS == bsp_efm_flash_read(address, length, p_data)) ? FAL_RESULT_SUCCESS
...
{
    (void)p_context;
    return (BSP_EFM_FLASH_RESULT_SUCCESS == bsp_efm_flash_program(address, length, p_data)) ? FAL_RESULT_SUCCESS
                                                                                            : FAL_RESULT_DRIVER_ERROR;
}
...
{
    (void)p_context;
    return (BSP_EFM_FLASH_RESULT_SUCCESS == bsp_efm_flash_erase(address, length)) ? FAL_RESULT_SUCCESS
                                                                                  : FAL_RESULT_DRIVER_ERROR;
}
...
{
    (void)p_context;
    return (BSP_W25Q64_RESULT_SUCCESS == bsp_w25q64_init()) ? FAL_RESULT_SUCCESS : FAL_RESULT_DRIVER_ERROR;
}
...
static fal_result_t w25q_read(void *p_context,
                              uint32_t address,
                              uint32_t length,
                              uint8_t *p_data)
{
    (void)p_context;
    return (BSP_W25Q64_RESULT_SUCCESS == bsp_w25q64_read(address, length, p_data)) ? FAL_RESULT_SUCCESS
                                                                                   : FAL_RESULT_DRIVER_ERROR;
}
...
{
    (void)p_context;
    return (BSP_W25Q64_RESULT_SUCCESS == bsp_w25q64_page_program(address, length, p_data)) ? FAL_RESULT_SUCCESS
                                                                                           : FAL_RESULT_DRIVER_ERROR;
}
...
static fal_result_t w25q_erase(void *p_context, uint32_t address, uint32_t length)
{
    ...
    return (BSP_W25Q64_RESULT_SUCCESS == bsp_w25q64_sector_erase(address)) ? FAL_RESULT_SUCCESS
                                                                           : FAL_RESULT_DRIVER_ERROR;
}
