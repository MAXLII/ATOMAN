...
static volatile bootloader_update_request_t boot_request HC32_BOOT_RETAINED; /* IAP request retained at the same address in both images. */
...
static bootloader_boot_reason_t boot_reason_get(void *p_context)
{
    ...
    (void)p_context;
    return (boot_request_read(&info) == 1u) ? BOOTLOADER_BOOT_REASON_IAP_REQUEST_E : BOOTLOADER_BOOT_REASON_POWER_ON_E;
}

static bootloader_result_t upgrade_info_get(void *p_context, bootloader_upgrade_info_t *p_info)
{
    (void)p_context;
    return (boot_request_read(p_info) == 1u) ? BOOTLOADER_RESULT_SUCCESS_E : BOOTLOADER_RESULT_IMAGE_INVALID_E;
}
...
static bootloader_result_t jump_to_iap(void *p_context)
{
    ...
    for (register_index = 0UL; register_index < (uint32_t)(sizeof(NVIC->ICER) / sizeof(NVIC->ICER[0]));
         register_index++)
    {
