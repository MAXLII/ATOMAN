                                                 ...
                                                 uint8_t *p_valid)
{
    ...
    if (    (p_header == NULL)
         || (p_valid == NULL)
         || (header_length < sizeof(reset_vector)))
    {
...
static bootloader_result_t jump_to_iap(void *p_context)
{
    ...
    if (    (remaining == 0u)
         || (remaining > ZYNQ7020_QSPI_IAP_SIZE))
    {
    ...
    while (remaining != 0u)
    {
        const uint32_t chunk = (remaining > BSP_QSPI_FLASH_MAX_READ) ? BSP_QSPI_FLASH_MAX_READ : remaining;

        if (bsp_qspi_flash_read(NULL, ZYNQ7020_QSPI_BOOT_SIZE + offset, chunk, &p_destination[offset]) != FAL_RESULT_SUCCESS)
        {
    ...
    Xil_DCacheFlushRange((INTPTR)ZYNQ7020_IAP_DDR_ADDRESS, p_platform->p_bootloader->upgrade_info.file_size);
