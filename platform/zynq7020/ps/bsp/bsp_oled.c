...
#define BSP_OLED_STATUS_BUSY_MASK \
    (BSP_OLED_STATUS_DMA_BUSY | BSP_OLED_STATUS_PROTOCOL_BUSY | BSP_OLED_STATUS_COMMAND_PENDING)
...
int32_t bsp_oled_init(void)
{
    ...
    status = wait_status(BSP_OLED_STATUS_INITIALIZED, BSP_OLED_STATUS_INITIALIZED);
    ...
    oled_initialized = 1U;
    bsp_oled_present();
    return ((register_read(BSP_OLED_REG_STATUS) & (BSP_OLED_STATUS_HALTED | BSP_OLED_STATUS_ERROR)) == 0U)
             ? XST_SUCCESS
             : XST_FAILURE;
}
...
{
    volatile uint8_t *p_framebuffer = (volatile uint8_t *)(UINTPTR)BSP_OLED_FRAMEBUFFER_BASE; /* DDR frame. */
...
void bsp_oled_present(void)
{
    ...
    register_write(BSP_OLED_REG_CONTROL, control_levels_get() | BSP_OLED_CONTROL_PRESENT);
...
int32_t bsp_oled_display_clear(void)
{
    ...
    register_write(BSP_OLED_REG_CONTROL, control_levels_get() | BSP_OLED_CONTROL_CLEAR);
...
int32_t bsp_oled_auto_refresh_set(uint8_t enabled, uint32_t period_ms)
{
    ...
    register_write(BSP_OLED_REG_CONTROL, control);
    return (enabled == 0U) ? wait_status(BSP_OLED_STATUS_BUSY_MASK, 0U) : XST_SUCCESS;
}
...
{
    uint32_t control = BSP_OLED_CONTROL_ENABLE | BSP_OLED_CONTROL_DISPLAY_ON; /* Restored default levels. */

    oled_initialized = 0U;
    register_write(BSP_OLED_REG_CONTROL, control_levels_get() | BSP_OLED_CONTROL_SOFT_RESET);
    register_write(BSP_OLED_REG_FB_BASE, BSP_OLED_FRAMEBUFFER_BASE);
    register_write(BSP_OLED_REG_SPI_DIV, BSP_OLED_DEFAULT_SPI_DIV);
    register_write(BSP_OLED_REG_REFRESH_PERIOD, BSP_OLED_DEFAULT_REFRESH_CYCLES);
    register_write(BSP_OLED_REG_CONTRAST, BSP_OLED_DEFAULT_CONTRAST);
    ...
    if (wait_status(BSP_OLED_STATUS_INITIALIZED, BSP_OLED_STATUS_INITIALIZED) != XST_SUCCESS)
    {
        return XST_FAILURE;
    }
    oled_initialized = 1U;
