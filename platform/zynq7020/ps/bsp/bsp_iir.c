...
uint8_t bsp_iir_limit_configure(int32_t lower_limit, int32_t upper_limit)
{
    ...
    bsp_iir_write_register((uint32_t)BSP_IIR_LIMIT_LOWER_OFFSET, (uint32_t)lower_limit);
    bsp_iir_write_register((uint32_t)BSP_IIR_LIMIT_UPPER_OFFSET, (uint32_t)upper_limit);
...
{
    bsp_iir_write_register((uint32_t)BSP_IIR_CONTROL_OFFSET, (uint32_t)BSP_IIR_CONTROL_RESET_STATE); /* 产生一个同步状态复位脉冲。 */
}
                               ...
                               uint32_t timeout_poll_count)
{
    ...
    bsp_iir_write_register((uint32_t)BSP_IIR_CONTROL_OFFSET, (uint32_t)BSP_IIR_CONTROL_START);
    ...
    *output_sample = (int32_t)bsp_iir_read_register((uint32_t)BSP_IIR_OUTPUT_OFFSET);
    ...
    bsp_iir_write_register((uint32_t)BSP_IIR_CONTROL_OFFSET, (uint32_t)BSP_IIR_CONTROL_CLEAR_DONE);
...
uint8_t bsp_iir_self_test(bsp_iir_self_test_result_t *result)
{
    ...
    int32_t saved_limit_lower = (int32_t)bsp_iir_read_register((uint32_t)BSP_IIR_LIMIT_LOWER_OFFSET); /* 自测前下限。 */
    int32_t saved_limit_upper = (int32_t)bsp_iir_read_register((uint32_t)BSP_IIR_LIMIT_UPPER_OFFSET); /* 自测前上限。 */
    ...
    for (sample_index = 0U; sample_index < BSP_IIR_SELF_TEST_SAMPLE_COUNT; ++sample_index)
    {
        const int32_t input_sample = (sample_index == 0U) ? (int32_t)BSP_IIR_TEST_IMPULSE : 0L;

        if (bsp_iir_process_sample(input_sample, &output_sample, &status, BSP_IIR_SELF_TEST_TIMEOUT) == 0U)
        {
            passed = 0U;
            break;
        }
    ...
    if (bsp_iir_read_register((uint32_t)BSP_IIR_SAMPLE_COUNT_OFFSET) != (uint32_t)BSP_IIR_SELF_TEST_SAMPLE_COUNT)
    {
        passed = 0U;
    }
    ...
    if (    (bsp_iir_read_register((uint32_t)BSP_IIR_X1_OFFSET) != 0U)
         ...
         || (bsp_iir_read_register((uint32_t)BSP_IIR_Y1_OFFSET) != (uint32_t)s_impulse_expected[7])
         || (bsp_iir_read_register((uint32_t)BSP_IIR_Y2_OFFSET) != (uint32_t)s_impulse_expected[6])
         || (bsp_iir_read_register((uint32_t)BSP_IIR_Y3_OFFSET) != (uint32_t)s_impulse_expected[5]))
    {
        passed = 0U;
    }
    ...
    if (    (bsp_iir_process_sample(INT32_MAX, &output_sample, &status, BSP_IIR_SELF_TEST_TIMEOUT) == 0U)
    ...
    if (    (bsp_iir_process_sample(INT32_MIN, &output_sample, &status, BSP_IIR_SELF_TEST_TIMEOUT) == 0U)
    ...
    if (    (bsp_iir_process_sample(1000L, &output_sample, &status, BSP_IIR_SELF_TEST_TIMEOUT) == 0U)
         || (output_sample != 100L)
         || ((status & (uint32_t)BSP_IIR_STATUS_SATURATED) == 0U)
    ...
    if (    (bsp_iir_process_sample(0L, &output_sample, &status, BSP_IIR_SELF_TEST_TIMEOUT) == 0U)
         || (output_sample != 100L)
         || ((status & (uint32_t)BSP_IIR_STATUS_SATURATED) != 0U))
    {
        passed = 0U;
    ...
    if (    (bsp_iir_process_sample(-1000L, &output_sample, &status, BSP_IIR_SELF_TEST_TIMEOUT) == 0U)
         || (output_sample != -75L)
         || ((status & (uint32_t)BSP_IIR_STATUS_SATURATED) == 0U)
         || (bsp_iir_read_register((uint32_t)BSP_IIR_Y1_OFFSET) != (uint32_t)-75L))
    {
        passed = 0U;
    }
