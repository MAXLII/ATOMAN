 ...
 *          - Execute blocking single-sample transactions
 *          - Verify feedforward, feedback, history, and saturation behavior
 *
 *          Design notes:
 *          - C11 compatible
...
#define BSP_IIR_MAX_OFFSET BSP_IIR_Y3_OFFSET /* 最后一个有效寄存器偏移。 */
...
#define BSP_IIR_STATUS_BUSY 0x00000001UL      /* PL 正在进行七次 MAC。 */
#define BSP_IIR_STATUS_DONE 0x00000002UL      /* 当前输出已更新。 */
#define BSP_IIR_STATUS_SATURATED 0x00000004UL /* 当前输出发生 32 位饱和。 */
#define BSP_IIR_STATUS_READY 0x00000008UL     /* 外设可以接收 START。 */

#define BSP_IIR_VERSION_VALUE 0x00010000UL /* 当前 RTL 版本。 */
 ...
 * @param[in] offset 寄存器字节偏移，必须 4 字节对齐且不大于 0x4C。
