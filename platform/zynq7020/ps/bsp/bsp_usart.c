...
{
    uint64_t numerator = (uint64_t)baud_rate * BSP_USART_PL_OVERSAMPLE * 0x100000000ULL;

    return (uint32_t)((numerator + (BSP_USART_PL_CLOCK_HZ / 2ULL)) / BSP_USART_PL_CLOCK_HZ);
}
...
static void pl_uart_error_interrupt_handler(void *callback_ref)
{
    ...
    (void)callback_ref;
    pending = pl_register_read(BSP_USART_PL_REG_IRQ_STATUS) & BSP_USART_PL_ERROR_IRQ_MASK;
...
uint8_t bsp_usart_dbg_tx_is_idle(void)
{
    ...
    return ((XUartPs_ReadReg(XPAR_XUARTPS_0_BASEADDR, XUARTPS_SR_OFFSET) & XUARTPS_SR_TXEMPTY) != 0U) ? 1U : 0U;
}
...
int32_t bsp_usart_pl_configure(const bsp_usart_pl_config_t *config)
{
    ...
    uart_config = (uint32_t)config->data_bits | ((uint32_t)config->parity << 4U);
    ...
    pl_register_write(BSP_USART_PL_REG_BAUD_INCREMENT, pl_baud_increment_get(config->baud_rate));
    ...
    if ((pl_register_read(BSP_USART_PL_REG_STATUS) & BSP_USART_PL_STATUS_ENABLED) == 0U)
    {
        return XST_FAILURE;
    }
...
int32_t bsp_usart_pl_reset(void)
{
    ...
    pl_register_write(BSP_USART_PL_REG_CONTROL, BSP_USART_PL_CONTROL_SOFT_RESET);
    pl_memory_barrier();
    s_pl_rx_consumed       = 0U;
    s_pl_tx_produced       = 0U;
...
int32_t bsp_usart_pl_init(void)
{
    ...
    if (pl_register_read(BSP_USART_PL_REG_VERSION) != BSP_USART_PL_EXPECTED_VERSION)
    {
        return XST_FAILURE;
    }

    status = bsp_interrupt_connect(BSP_USART_PL_IRQ_ID, (Xil_ExceptionHandler)pl_uart_error_interrupt_handler, NULL);
...
{
    volatile uint8_t *tx_ring = (volatile uint8_t *)(UINTPTR)BSP_USART_PL_TX_RING_BASE;
    ...
    while (sent < length)
    {
        uint32_t tx_consumed = pl_register_read(BSP_USART_PL_REG_TX_CONSUMED);
        ...
        while (    (available != 0U)
                && (sent < length))
        {
            tx_ring[s_pl_tx_produced & (BSP_USART_PL_RING_SIZE - 1U)] = data[sent];
            s_pl_tx_produced++;
            sent++;
            available--;
        }
...
{
    volatile const uint8_t *rx_ring = (volatile const uint8_t *)(UINTPTR)BSP_USART_PL_RX_RING_BASE;
