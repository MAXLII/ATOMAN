...
fal_result_t bsp_qspi_flash_init(void *p_context)
{
    ...
    if (
     ...
     || (XQspiPs_SetOptions(&p_flash->instance, XQSPIPS_MANUAL_START_OPTION | XQSPIPS_FORCE_SSELECT_OPTION | XQSPIPS_HOLD_B_DRIVE_OPTION) != XST_SUCCESS)
...
fal_device_state_t bsp_qspi_flash_state_get(void *p_context)
{
    ...
    if (transfer(p_flash, status_command, status_response, (uint32_t)sizeof(status_command)) != FAL_RESULT_SUCCESS)
    {
        p_flash->operation_pending = 0u;
        return FAL_DEVICE_STATE_ERROR;
    }
                                 ...
                                 uint8_t *p_data)
{
    ...
    if (transfer(p_flash, p_flash->transfer_buffer, p_flash->receive_buffer, length + QSPI_COMMAND_SIZE) != FAL_RESULT_SUCCESS)
    {
        return FAL_RESULT_DRIVER_ERROR;
    }
                                    ...
                                    const uint8_t *p_data)
{
    ...
    if (    (p_data == NULL)
         ...
         || ((address / BSP_QSPI_FLASH_PAGE_SIZE) != ((address + length - 1u) / BSP_QSPI_FLASH_PAGE_SIZE))
         || (address > BSP_QSPI_FLASH_CAPACITY)
         || (length > (BSP_QSPI_FLASH_CAPACITY - address))
         || (p_flash->operation_pending != 0u))
    {
    ...
    if (transfer(p_flash, p_flash->transfer_buffer, NULL, length + QSPI_COMMAND_SIZE) != FAL_RESULT_SUCCESS)
    {
        return FAL_RESULT_DRIVER_ERROR;
    }
