                            ...
                            err_t error)
{
    ...
    for (p_fragment = p_packet; p_fragment != NULL; p_fragment = p_fragment->next)
    {
        ...
        for (index = 0U; index < p_fragment->len; index++)
        {
            ...
            s_ethernet.rx_write_index = (s_ethernet.rx_write_index + 1U) % BSP_ETHERNET_RX_BUFFER_SIZE;
        }
    }
...
int32_t bsp_ethernet_init(void)
{
    ...
    if (xemac_add(&s_ethernet.network_interface, &address, &netmask, &gateway, mac_address, (UINTPTR)XPAR_XEMACPS_0_BASEADDR) == NULL)
    {
        return XST_FAILURE;
    }
...
uint8_t bsp_ethernet_rx_get_byte(uint8_t *p_data)
{
    ...
    *p_data = s_ethernet.rx_buffer[s_ethernet.rx_read_index];
    s_ethernet.rx_read_index = (s_ethernet.rx_read_index + 1U) % BSP_ETHERNET_RX_BUFFER_SIZE;
    s_ethernet.rx_count--;
    s_ethernet.status.receive_buffer_used = s_ethernet.rx_count;
    return 1U;
}
...
int32_t bsp_ethernet_tx(const uint8_t *p_data, uint32_t length)
{
    ...
    while (offset < length)
    {
        ...
        error = tcp_write(p_client, &p_data[offset], (u16_t)chunk_length, (u8_t)TCP_WRITE_FLAG_COPY);
