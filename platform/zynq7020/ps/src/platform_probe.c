...
static uint8_t pl_dma_loopback_test(bsp_usart_pl_status_t *final_status)
{
    ...
    while (    (passed != 0U)
            && (sent < PLATFORM_PL_DMA_TEST_BYTES))
    {
        ...
        {
            tx_lfsr = (tx_lfsr >> 1U) ^ ((0U - (tx_lfsr & 1U)) & 0xD0000001U);
    ...
    while (    (passed != 0U)
            && (received < PLATFORM_PL_DMA_TEST_BYTES))
    {
        uint8_t actual = 0U;

        if (bsp_usart_pl_rx_get_byte(&actual) != 0U)
        {
            rx_lfsr = (rx_lfsr >> 1U) ^ ((0U - (rx_lfsr & 1U)) & 0xD0000001U);
        ...
        else if ((bsp_timer_gettime_100us() - start_tick) >= PLATFORM_PL_DMA_TIMEOUT_TICKS)
        {
            passed = 0U;
        }
    }
...
static void self_test_tx_capture(char *data, int length)
{
    ...
    copy_length =
        ((uint32_t)length > PLATFORM_SELF_TEST_FRAME_SIZE) ? (uint16_t)PLATFORM_SELF_TEST_FRAME_SIZE : (uint16_t)length;
...
static void platform_self_test_report(void)
{
    ...
    bsp_usart_dbg_printf(
        "[SELFTEST] mode=%s result=%s reg(init=%lu task=%lu irq=%lu shell=%lu link=%lu perf=%lu comm=%lu scope=%lu sfra=%lu) runtime(task=%lu irq=%u shell=%lu perf=%u scope=%u sfra=%u trace=%lu comm_loop=%u shell_loop=%u srtos=%lu iir=%u pl_dma=%u/%lu)\r\n",
...
static void platform_probe_init(void)
{
    ...
    bsp_usart_dbg_printf(
        "commands: help, ZYNQ_STATUS, IIR_TEST, PL_UART_STATUS, PL_UART_RESET, PL_UART_DMA_TEST, PL_UART_ERROR_CLEAR, PL_OLED_STATUS, PL_OLED_CLEAR, PL_OLED_TEST, PL_OLED_RESET, PL_OLED_INVERT, PL_OLED_CONTRAST\r\n");
}
...
static void platform_status_command(DEC_MY_PRINTF)
{
    ...
    my_printf->my_printf(
        "zynq mode=%s tick_100us=%lu task_count=%lu section=%08lX-%08lX srtos=%lu fault=%lu save_fail=%lu release_fail=%lu pool=%lu/%lu stack_free=%lu\r\n",
        section_runtime_name_get(),
...
static void platform_iir_test_command(DEC_MY_PRINTF)
{
    ...
    my_printf->my_printf(
        "iir result=%s impulse=%ld,%ld,%ld,%ld,%ld,%ld,%ld,%ld sat=%ld/%ld limits=%ld/%ld feedback=%ld count=%lu status=%08lX version=%08lX format=%08lX\r\n",
        (passed == 1U) ? "PASS" : "FAIL",
...
static void platform_pl_uart_status_command(DEC_MY_PRINTF)
{
    ...
    my_printf->my_printf(
        "pl_uart version=%08lX status=%08lX irq=%08lX/%lu/%08lX rx=%lu/%lu tx=%lu/%lu uart_err=%08lX dma_err=%08lX stop=%08lX\r\n",
...
static void platform_pl_uart_dma_test_command(DEC_MY_PRINTF)
{
    ...
    {
        my_printf->my_printf(
            "pl_uart_dma_test result=%s bytes=%lu rx=%lu/%lu tx=%lu/%lu irq=%lu errors=%08lX/%08lX stop=%08lX\r\n",
            (passed != 0U) ? "PASS" : "FAIL",
...
static void platform_pl_oled_status_command(DEC_MY_PRINTF)
{
    ...
    my_printf->my_printf(
        "pl_oled version=%08lX status=%08lX fb=%08lX frame=%lu clear=%lu irq=%08lX/%lu/%08lX errors=%lu/%lu stop=%08lX\r\n",
        (unsigned long)status.version,
        (unsigned long)status.status,
...
static void platform_pl_oled_test_command(DEC_MY_PRINTF)
{
    ...
    for (y = 0U; y < BSP_OLED_HEIGHT; ++y)
    {
        ...
        {
            uint8_t enabled = (uint8_t)((((x >> 3U) + (y >> 3U)) & 1U) == 0U); /* 8x8 checkerboard pixel. */
    ...
    {
        my_printf->my_printf(
            "pl_oled_test result=%s pattern=checkerboard manual=%lu auto100=%lu auto1000=%lu irq=%lu errors=%lu/%lu stop=%08lX\r\n",
            (passed == 1U) ? "PASS" : "FAIL",
            (unsigned long)manual_frame_count,
            (unsigned long)(auto_1000_start_count - auto_100_start_count),
            (unsigned long)(status.frame_count - auto_1000_start_count),
            (unsigned long)status.error_irq_count,
            (unsigned long)status.axi_error_count,
            (unsigned long)status.command_error_count,
            (unsigned long)status.dma_stop_reason);
