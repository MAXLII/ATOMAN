...
{
    spi->CTRL1 &= 0x3040;
    spi->CTRL1 |= (uint16_t)((uint32_t)spiConfig->direction | spiConfig->mode | spiConfig->length | spiConfig->polarity
                             | spiConfig->phase | spiConfig->nss | spiConfig->baudrateDiv | spiConfig->firstBit);
...
void I2S_Config(SPI_T *spi, I2S_Config_T *i2sConfig)
{
    ...
    spi->I2SCFG = (uint32_t)i2sConfig->mode | (uint32_t)i2sConfig->standard | (uint32_t)i2sConfig->length
 ...
 *
 * @param spi: The SPIx can be 1,2, When the I2S can be 2,3
 *
