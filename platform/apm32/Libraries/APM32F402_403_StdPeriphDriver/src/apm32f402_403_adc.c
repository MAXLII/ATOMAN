...
void ADC_Config(ADC_T *adc, ADC_Config_T *adcConfig)
{
    ...
    reg = adc->CTRL2;
    reg &= 0xFFF1F7FD;
    reg |= (uint32_t)adcConfig->dataAlign | (uint32_t)adcConfig->externalTrigConv
         | ((uint32_t)adcConfig->continuousConvMode << 1);
    adc->CTRL2 = reg;
...
uint8_t ADC_ReadIntFlag(ADC_T *adc, ADC_INT_T flag)
{
    ...
    if (    ((adc->STS & itmask) != (uint32_t)RESET)
         && enableStatus)
    {
        bitStatus = SET;
    }
