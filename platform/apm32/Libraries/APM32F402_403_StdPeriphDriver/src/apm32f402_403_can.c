...
uint8_t CAN_Config(CAN_T *can, CAN_Config_T *canConfig)
{
    ...
    while (    ((can->MSTS_B.INITFLG) != BIT_SET)
            && (wait_ack != 0x0000FFFF))
    {
        wait_ack++;
    }
    ...
    else
    {
        ...
        while (    ((can->MSTS_B.INITFLG) != BIT_RESET)
                && (wait_ack != 0x0000FFFF))
        {
            wait_ack++;
        }
...
void CAN_ConfigFilter(CAN_FilterConfig_T *filterConfig)
{
    ...
    if (filterConfig->filterScale == CAN_FILTER_SCALE_16BIT)
    {
        ...
        CAN1->sFilterRegister[filterConfig->filterNumber].FBANK1 =
            ((0x0000FFFF & filterConfig->filterMaskIdLow) << 16) | (0x0000FFFF & filterConfig->filterIdLow);

        CAN1->sFilterRegister[filterConfig->filterNumber].FBANK2 =
            ((0x0000FFFF & filterConfig->filterMaskIdHigh) << 16) | (0x0000FFFF & filterConfig->filterIdHigh);
    }
    ...
    if (filterConfig->filterScale == CAN_FILTER_SCALE_32BIT)
    {
        ...
        CAN1->sFilterRegister[filterConfig->filterNumber].FBANK1 =
            ((0x0000FFFF & filterConfig->filterIdHigh) << 16) | (0x0000FFFF & filterConfig->filterIdLow);

        CAN1->sFilterRegister[filterConfig->filterNumber].FBANK2 =
            ((0x0000FFFF & filterConfig->filterMaskIdHigh) << 16) | (0x0000FFFF & filterConfig->filterMaskIdLow);
    }
...
uint8_t CAN_TxMessage(CAN_T *can, CAN_TxMessage_T *TxMessage)
{
    ...
    {
        can->sTxMailBox[transmit_milbox].TXMID |=
            (TxMessage->extID << 3) | (TxMessage->typeID) | (TxMessage->remoteTxReq);
    }
...
uint8_t CAN_WakeUpMode(CAN_T *can)
{
    ...
    while (    (can->MSTS_B.SLEEPFLG == BIT_SET)
            && (time_out != 0))
    {
        time_out--;
    }
