...
void USB_OTG_SetTurnaroundTime(USB_OTG_GLOBAL_T *usbx, uint32_t hclkFreq, uint8_t speed)
{
    ...
    case USB_OTG_SPEED_FSLS:
        ...
        else if (    (hclkFreqTemp >= 150)
                  && (hclkFreqTemp < 160))
        {
            turnaroundTime = 0x0E;
        }
        else if (    (hclkFreqTemp >= 160)
                  && (hclkFreqTemp < 172))
        {
            turnaroundTime = 0x0D;
        }
        else if (    (hclkFreqTemp >= 172)
                  && (hclkFreqTemp < 185))
        {
            turnaroundTime = 0x0C;
        }
        else if (    (hclkFreqTemp >= 185)
                  && (hclkFreqTemp < 200))
        {
            turnaroundTime = 0x0B;
        }
        else if (    (hclkFreqTemp >= 200)
                  && (hclkFreqTemp < 218))
        {
            turnaroundTime = 0x0A;
        }
        else if (    (hclkFreqTemp >= 218)
                  && (hclkFreqTemp < 240))
        {
            turnaroundTime = 0x09;
        }
        else if (    (hclkFreqTemp >= 240)
                  && (hclkFreqTemp < 277))
        {
            turnaroundTime = 0x08;
        }
        else if (    (hclkFreqTemp >= 277)
                  && (hclkFreqTemp < 320))
        {
            turnaroundTime = 0x07;
        }
 ...
 * @retval None
 */
void USB_OTG_FIFO_WriteFifoPacket(USB_OTG_FIFO_T *usbFifo,
                                  uint8_t chEpNum,
                                  uint8_t *wBuf,
                                  uint32_t wlen,
                                  uint8_t dmaStatus)
