...
static void TI1Config(TMR_T *tmr,
                      uint16_t ICpolarity,
                      uint16_t ICselection,
                      uint16_t ICfilter);
static void TI2Config(TMR_T *tmr,
                      uint16_t ICpolarity,
                      uint16_t ICselection,
                      uint16_t ICfilter);
static void TI3Config(TMR_T *tmr,
                      uint16_t ICpolarity,
                      uint16_t ICselection,
                      uint16_t ICfilter);
static void TI4Config(TMR_T *tmr,
                      uint16_t ICpolarity,
                      uint16_t ICselection,
                      uint16_t ICfilter);
 ...
 *
 * @param tmr: Clear TMRx peripheral, The TMRx can be 1 to 8 except 6 and 7
 *
 * @retval None
 *
 */
...
{
    tmr->BDT = (BDTConfig->IMOS) << 10 | (BDTConfig->RMOS) << 11 | (BDTConfig->lockLevel) << 8 | (BDTConfig->deadTime)
             | (BDTConfig->BRKState) << 12 | (BDTConfig->BRKPolarity) << 13 | (BDTConfig->automaticOutput) << 14;
}
 ...
 * @retval None
 */
void TMR_ConfigETRClockMode1(TMR_T *tmr,
                             TMR_EXTTRG_PSC_T prescaler,
                             TMR_EXTTRG_POL_T polarity,
                             uint16_t filter)
{
 ...
 * @retval None
 */
void TMR_ConfigETRClockMode2(TMR_T *tmr,
                             TMR_EXTTRG_PSC_T prescaler,
                             TMR_EXTTRG_POL_T polarity,
                             uint16_t filter)
{
 ...
 * @retval None
 */
void TMR_ConfigEncodeInterface(TMR_T *tmr,
                               TMR_ENCODER_MODE_T encodeMode,
                               TMR_IC_POLARITY_T IC1Polarity,
...
/*!
 * @brief Config TMR remap.
 *
 * @param tmr: Only for TMR2
 *
 * @param remap: Internal Trigger 1 Remap Select
 *        The parameter can be any combination following values:
 * @arg TMR2_REMAP_TMR8_TRGO         : TMR8 TRGOUT
 * @arg TMR2_REMAP_OTG_FS_SOF_ITR1   : OTG_FS SOF connected to TMR2_ITR1 input
 *
 * @retval Counter Register value.
 */
void TMR_ConfigRemap(TMR_T *tmr, TMR_REMAP_T remap)
{
    tmr->OPT = remap;
}

/*!
 * @brief Read TMR remap.
 *
 * @param tmr: Only for TMR2
 *
 * @retval Internal Trigger 1 Remap Select
 *         The parameter can be any combination following values:
 * @arg TMR2_REMAP_TMR8_TRGO         : TMR8 TRGOUT
 * @arg TMR2_REMAP_OTG_FS_SOF_ITR1   : OTG_FS SOF connected to TMR2_ITR1 input
 */
uint16_t TMR_ReadRemap(TMR_T *tmr)
{
    return tmr->OPT;
}
