...
static uint8_t frame_sfra_name_length(const char *p_name)
{
    ...
    while (    (length < FRAME_SFRA_NAME_MAX)
            && (p_name[length] != '\0'))
    {
        length++;
    }
...
static void frame_sfra_list_task(void)
{
    ...
    if (s_sfra_list.p_item == NULL)
    {
        payload[0] = 0xFFu;
        payload[1] = 1u;
        frame_sfra_route_send(&s_sfra_list.route, CMD_WORD_SFRA_LIST_QUERY, 1u, payload, FRAME_SFRA_LIST_HEADER_SIZE);
    ...
    {
        payload[FRAME_SFRA_LIST_HEADER_SIZE + index] = wire_octet_get((uint16_t)p_registration->p_name[index]);
    }
...
static void frame_sfra_info_query_act(void *p_frame, DEC_MY_PRINTF)
{
    ...
    if (    (p_pack->p_data != NULL)
         && (p_pack->len >= FRAME_SFRA_INFO_REQUEST_SIZE))
    {
    ...
    frame_sfra_reply(p_pack, my_printf, CMD_WORD_SFRA_INFO_QUERY, payload, FRAME_SFRA_INFO_ACK_SIZE);
}
...
static void frame_sfra_config_act(void *p_frame, DEC_MY_PRINTF)
{
    ...
    if (    (p_pack->p_data == NULL)
         || (p_pack->len < FRAME_SFRA_CONFIG_REQUEST_SIZE))
    {
        frame_sfra_control_encode(payload, NULL, sfra_id, status);
        frame_sfra_reply(p_pack, my_printf, CMD_WORD_SFRA_CFG_SET, payload, FRAME_SFRA_CONTROL_ACK_SIZE);
        return;
    }
    ...
    else
    {
        ...
        if (frame_sfra_is_busy(p_sfra) != 0u)
        {
            status = (uint8_t)SFRA_TOOL_STATUS_BUSY;
                  ...
                  || ((apply_mask & (uint8_t)~(SFRA_CFG_APPLY_FREQ | SFRA_CFG_APPLY_AMPLITUDE)) != 0u)
...
static void frame_sfra_control_act(void *p_frame, DEC_MY_PRINTF)
{
    ...
    if (    (p_pack->p_data != NULL)
         && (p_pack->len >= FRAME_SFRA_CONTROL_REQUEST_SIZE))
    {
        sfra_id        = wire_octet_get(p_pack->p_data[0]);
        p_registration = frame_sfra_find(sfra_id);
    }
    ...
    frame_sfra_control_encode(payload, p_registration, sfra_id, status);
    frame_sfra_reply(p_pack, my_printf, wire_octet_get(p_pack->cmd_word), payload, FRAME_SFRA_CONTROL_ACK_SIZE);
}
...
static void frame_sfra_point_query_act(void *p_frame, DEC_MY_PRINTF)
{
    ...
    if (    (p_pack->p_data != NULL)
         && (p_pack->len >= FRAME_SFRA_POINT_REQUEST_SIZE))
    {
        sfra_id            = wire_octet_get(p_pack->p_data[0]);
    ...
    frame_sfra_point_encode(payload, p_registration, sfra_id, point_index, status);
    frame_sfra_reply(p_pack, my_printf, CMD_WORD_SFRA_POINT_QUERY, payload, FRAME_SFRA_POINT_ACK_SIZE);
}
