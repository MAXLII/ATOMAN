 ...
 *          - Configure LED4 and SCIA through C2000Ware DriverLib
 ...
 * @return Next queue index in the bounded storage range.
 */
#pragma CODE_SECTION(tms320f28p55_uart_tx_index_next, ".TI.ramfunc")
 ...
 */
#pragma CODE_SECTION(tms320f28p55_uart_rx_push, ".TI.ramfunc")
static uint8_t tms320f28p55_uart_rx_push(uint8_t data)
{
    ...
    {
        g_tms320f28p55_uart_rx_drop_count++;
        return 0u;
    }
...
static __interrupt void tms320f28p55_cpu_timer0_isr(void)
{
...
#pragma CODE_SECTION(tms320f28p55_scia_rx_isr, ".TI.ramfunc")
static __interrupt void tms320f28p55_scia_rx_isr(void)
{
           ...
           (processed_count < TMS320F28P55_SCIA_RX_FIFO_CAPACITY))
    {
        ...
        {
            g_tms320f28p55_uart_rx_octet_count++;
        }
        processed_count++;
    }
...
static void tms320f28p55_led_init(void)
{
...
static void tms320f28p55_uart_init(void)
{
    Interrupt_register(INT_SCIA_RX, &tms320f28p55_scia_rx_isr);
    Interrupt_register(INT_SCIA_TX, &tms320f28p55_scia_tx_isr);
    ...
    SCI_setConfig(SCIA_BASE,
                  TMS320F28P55_LSPCLK_HZ,
                  TMS320F28P55_UART_BAUD,
...
static void tms320f28p55_tick_init(void)
{
    const uint32_t timer_period = (TMS320F28P55_SYSCLK_HZ / TMS320F28P55_TICK_HZ) - 1u;

    Interrupt_register(INT_TIMER0, &tms320f28p55_cpu_timer0_isr);
 ...
 */
#pragma CODE_SECTION(tms320f28p55_perf_counter_init, ".TI.ramfunc")
static void tms320f28p55_perf_counter_init(void)
{
    uint16_t counter_control = 0u; /* Counter1 control value assembled while disabled. */
    ...
    HWREGH(ERAD_GLOBAL_BASE + ERAD_O_GLBL_ENABLE) |= (uint16_t)ERAD_INST_COUNTER1;
...
{
    tms320f28p55_led_init();
    tms320f28p55_uart_init();
    tms320f28p55_tick_init();
    tms320f28p55_perf_counter_init();
}
...
void tms320f28p55_uart_tx_write(const uint8_t *p_data, uint16_t length)
{
    ...
    if (length >= TMS320F28P55_UART_TX_QUEUE_CAPACITY)
    {
        g_tms320f28p55_uart_tx_drop_frame_count++;
        return;
    }

    while (tms320f28p55_uart_tx_free_get() < length)
    {
