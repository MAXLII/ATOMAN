...
/**
 * @file f28p55_shell_adapter.c
 * @brief F28P55 SECTION Shell registry adapter.
 * @details
...
void shell_init(void)
{
    ...
    for (p_section = (reg_section_t *)&SECTION_START; p_section < (reg_section_t *)&SECTION_STOP; p_section++)
    {
...
section_shell_t *shell_find(const char *p_name, uint8_t length)
{
    ...
    while (p_item != NULL)
    {
        ...
        if (    (p_shell != NULL)
             && (p_shell->p_name_size == (uint32_t)length))
        {
            for (index = 0u; index < (uint16_t)length; index++)
            {
                if (((uint16_t)p_shell->p_name[index] & 0x00FFu) != ((uint16_t)p_name[index] & 0x00FFu))
                {
                    matches = 0u;
                    break;
                }
