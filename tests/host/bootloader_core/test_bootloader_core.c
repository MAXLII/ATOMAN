...
typedef struct
{
    uint8_t zones[BOOTLOADER_FLASH_ZONE_COUNT_E][TEST_ZONE_SIZE];
...
typedef struct
{
    ...
    uint8_t upgrade_info_valid;
} fake_platform_t;

static FILE *g_log_file;
static uint32_t g_pass_count;
static uint32_t g_fail_count;
static fake_storage_t *p_fake_storage = NULL;
...
static bootloader_result_t fake_zone_info(bootloader_flash_zone_t zone,
                                          bootloader_flash_zone_info_t *p_info)
{
    if ((zone >= BOOTLOADER_FLASH_ZONE_COUNT_E) || (p_info == NULL))
    {
    ...
    *p_info = (bootloader_flash_zone_info_t){
        ...
        .readable = 1u,
        .writable = (uint8_t)((zone == BOOTLOADER_FLASH_ZONE_LAYOUT_E) ? 0u : 1u),
        .erasable = (uint8_t)((zone == BOOTLOADER_FLASH_ZONE_LAYOUT_E) ? 0u : 1u),
    };
    return BOOTLOADER_RESULT_SUCCESS_E;
}

static bootloader_result_t fake_read(bootloader_flash_zone_t zone,
                                     uint32_t offset,
                                     uint32_t length,
                                     uint8_t *p_data)
{
    fake_storage_t *p_storage = p_fake_storage;

    if ((zone >= BOOTLOADER_FLASH_ZONE_COUNT_E) || (p_data == NULL) ||
    ...
    if (zone == BOOTLOADER_FLASH_ZONE_IAP_E)
    {
    ...
    p_storage->result = BOOTLOADER_RESULT_SUCCESS_E;
    return BOOTLOADER_RESULT_SUCCESS_E;
}

static bootloader_result_t fake_write(bootloader_flash_zone_t zone,
                                      uint32_t offset,
                                      uint32_t length,
                                      const uint8_t *p_data)
{
    ...
    if (p_storage->fail_next_write != 0u)
    {
        ...
        p_storage->result = BOOTLOADER_RESULT_STORAGE_ERROR_E;
...
static bootloader_result_t fake_erase(bootloader_flash_zone_t zone,
                                      uint32_t offset,
                                      uint32_t length)
{
    fake_storage_t *p_storage = p_fake_storage;

    if ((zone >= BOOTLOADER_FLASH_ZONE_COUNT_E) ||
        (offset > TEST_ZONE_SIZE) || (length > (TEST_ZONE_SIZE - offset)))
    {
        return BOOTLOADER_RESULT_OUT_OF_RANGE_E;
    }
...
static uint8_t fake_is_busy(void)
{
    return 0u;
}

static bootloader_result_t fake_result_get(void)
{
    return p_fake_storage->result;
}
...
static bootloader_result_t fake_upgrade_info_get(void *p_context,
                                                 bootloader_upgrade_info_t *p_info)
{
    fake_platform_t *p_platform = (fake_platform_t *)p_context;

    if ((p_info == NULL) || (p_platform->upgrade_info_valid == 0u))
    {
        return BOOTLOADER_RESULT_IMAGE_INVALID_E;
    }
    *p_info = p_platform->upgrade_info;
    return BOOTLOADER_RESULT_SUCCESS_E;
}
...
static bootloader_platform_ops_t platform_ops_make(fake_platform_t *p_platform)
{
    return (bootloader_platform_ops_t){
        ...
        .p_upgrade_info_get = fake_upgrade_info_get,
                                           ...
                                           uint8_t *p_copy)
{
    ...
    if (bootloader_flash_ops_init(p_bootloader, &flash_ops) != BOOTLOADER_RESULT_SUCCESS_E)
    {
        return BOOTLOADER_RESULT_CONFIG_ERROR_E;
    }
    return bootloader_init(p_bootloader, &config, &platform_ops);
}
...
{
    bootloader_t bootloader = {0};     /* Bootloader instance exercised by each startup scenario. */
    fake_storage_t storage = {0};      /* Persistent fake Flash shared by reset scenarios. */
    fake_platform_t platform = {0};    /* Boot reason and retained information source. */
    bootloader_metadata_t stale_metadata = {
        .state = BOOTLOADER_METADATA_STATE_DOWNLOAD_STAGED,
        ...
        .file_size = 256u,
        .expected_crc = 0u,
        .received_length = 128u,
        .running_crc = 0u,
        .copy_offset = 0u,
        .error_code = 0u,
    }; /* Interrupted older download superseded by a newly retained IAP request. */
    uint8_t packet[BOOTLOADER_PACKET_DATA_SIZE] = {0}; /* Packet and startup-header scratch space. */
    uint8_t copy[300] = {0};                          /* Metadata and staged-copy scratch space. */

    (void)memset(&storage, 0, sizeof(storage));
    (void)memset(storage.zones, 0xFF, sizeof(storage.zones));
    (void)memset(&platform, 0, sizeof(platform));
    storage.result = BOOTLOADER_RESULT_SUCCESS_E;
    check_result(bootloader_make(&bootloader, &storage, &platform, packet, copy) == BOOTLOADER_RESULT_SUCCESS_E,
                 "blank image initialization succeeds");
    process_until(&bootloader, BOOTLOADER_STATE_WAIT_UPGRADE_E);
    check_result((bootloader_state_get(&bootloader) == BOOTLOADER_STATE_WAIT_UPGRADE_E) &&
                     (platform.jump_count == 0u),
                 "blank image remains in bootloader");

    storage.zones[BOOTLOADER_FLASH_ZONE_IAP_E][0] = 0x5Au;
    storage.zones[BOOTLOADER_FLASH_ZONE_IAP_E][1] = 0xA5u;
    (void)memset(&platform, 0, sizeof(platform));
    check_result(bootloader_make(&bootloader, &storage, &platform, packet, copy) == BOOTLOADER_RESULT_SUCCESS_E,
                 "valid image initialization succeeds");
    process_until(&bootloader, BOOTLOADER_STATE_JUMP_PENDING_E);
    ...
    platform.reason = BOOTLOADER_BOOT_REASON_IAP_REQUEST_E;
    platform.jump_count = 0u;
    check_result(bootloader_make(&bootloader, &storage, &platform, packet, copy) == BOOTLOADER_RESULT_SUCCESS_E,
    ...
    (void)memset(storage.zones[BOOTLOADER_FLASH_ZONE_META_A_E], 0xFF, TEST_ZONE_SIZE);
    (void)memset(storage.zones[BOOTLOADER_FLASH_ZONE_META_B_E], 0xFF, TEST_ZONE_SIZE);
    (void)bootloader_metadata_encode(&stale_metadata,
                                     storage.zones[BOOTLOADER_FLASH_ZONE_META_A_E],
                                     TEST_ZONE_SIZE);
    platform.upgrade_info = (bootloader_upgrade_info_t){
        .module_id = 2u,
        .file_size = 512u,
        .version = 0x01020304u,
        .update_type = 1u,
    };
    platform.upgrade_info_valid = 1u;
    check_result(bootloader_make(&bootloader, &storage, &platform, packet, copy) ==
                     BOOTLOADER_RESULT_SUCCESS_E,
                 "retained IAP upgrade information is accepted");
    process_until(&bootloader, BOOTLOADER_STATE_DOWNLOAD_READY_E);
    check_result((bootloader_state_get(&bootloader) == BOOTLOADER_STATE_DOWNLOAD_READY_E) &&
                     (bootloader.upgrade_info.file_size == 512u) &&
                     (bootloader.upgrade_info.version == 0x01020304u),
                 "retained IAP request replaces stale download metadata without another 0x08");
}
...
static void test_staged_upgrade(void)
{
    ...
    fake_platform_t platform = {.reason = BOOTLOADER_BOOT_REASON_IAP_REQUEST_E};
    ...
    check_result(bootloader_make(&bootloader, &storage, &platform, packet_buffer, copy_buffer) == BOOTLOADER_RESULT_SUCCESS_E,
                 "staged upgrade initialization succeeds");
    process_until(&bootloader, BOOTLOADER_STATE_WAIT_UPGRADE_E);
    check_result(bootloader_upgrade_begin(&bootloader, &info, BOOTLOADER_UPGRADE_MODE_STAGED_E) == BOOTLOADER_RESULT_SUCCESS_E,
    ...
    while (offset < (uint32_t)sizeof(image))
    {
        ...
        check_result(bootloader_packet_submit(&bootloader, offset, &image[offset], length) == BOOTLOADER_RESULT_SUCCESS_E,
    ...
    check_result(bootloader_upgrade_end(&bootloader, crc) == BOOTLOADER_RESULT_SUCCESS_E,
                 "verified staged image starts installation");
    process_until(&bootloader, BOOTLOADER_STATE_JUMP_PENDING_E);
    check_result(memcmp(storage.zones[BOOTLOADER_FLASH_ZONE_IAP_E], image, sizeof(image)) == 0,
...
static void test_direct_failure(void)
{
    ...
    process_until(&bootloader, BOOTLOADER_STATE_WAIT_UPGRADE_E);
    check_result(bootloader_upgrade_begin(&bootloader, &info, BOOTLOADER_UPGRADE_MODE_DIRECT_E) == BOOTLOADER_RESULT_SUCCESS_E,
                 "direct upgrade session starts");
    process_until(&bootloader, BOOTLOADER_STATE_DOWNLOAD_READY_E);
    storage.fail_next_write = 1u;
    check_result(bootloader_packet_submit(&bootloader, 0u, data, (uint32_t)sizeof(data)) == BOOTLOADER_RESULT_STORAGE_ERROR_E,
...
static void test_copy_readback_failure(void)
{
    ...
    (void)bootloader_make(&bootloader, &storage, &platform, packet_buffer, copy_buffer);
    process_until(&bootloader, BOOTLOADER_STATE_WAIT_UPGRADE_E);
    (void)bootloader_upgrade_begin(&bootloader, &info, BOOTLOADER_UPGRADE_MODE_STAGED_E);
    process_until(&bootloader, BOOTLOADER_STATE_DOWNLOAD_READY_E);
    ...
    process_until(&bootloader, BOOTLOADER_STATE_WAIT_UPGRADE_E);
    check_result((bootloader_result_get(&bootloader) == BOOTLOADER_RESULT_IMAGE_INVALID_E) &&
                     (platform.jump_count == 0u),
...
static void test_staged_power_loss_recovery(void)
{
    ...
    process_until(&first_boot, BOOTLOADER_STATE_WAIT_UPGRADE_E);
    (void)bootloader_upgrade_begin(&first_boot, &info, BOOTLOADER_UPGRADE_MODE_STAGED_E);
    process_until(&first_boot, BOOTLOADER_STATE_DOWNLOAD_READY_E);
    (void)bootloader_packet_submit(&first_boot, 0u, image, (uint32_t)sizeof(image));
    process_until(&first_boot, BOOTLOADER_STATE_DOWNLOAD_READY_E);
    (void)bootloader_upgrade_end(&first_boot, crc);
    process_until(&first_boot, BOOTLOADER_STATE_COPY_ERASE_E);

    platform.reason = BOOTLOADER_BOOT_REASON_POWER_ON_E;
    platform.jump_count = 0u;
    check_result(bootloader_make(&recovered_boot, &storage, &platform,
                                 recovered_packet, recovered_copy) == BOOTLOADER_RESULT_SUCCESS_E,
                 "power-loss restart initializes from redundant metadata");
    process_until(&recovered_boot, BOOTLOADER_STATE_JUMP_PENDING_E);
    check_result(memcmp(storage.zones[BOOTLOADER_FLASH_ZONE_IAP_E], image, sizeof(image)) == 0,
...
static void test_protocol(void)
{
    ...
    check_result(bootloader_protocol_init(&protocol, &bootloader, test_crc_calculate,
                                          BOOTLOADER_UPGRADE_MODE_DIRECT_E) == BOOTLOADER_RESULT_SUCCESS_E,
    ...
    check_result(bootloader_protocol_handle(&protocol, BOOTLOADER_PROTOCOL_CMD_INFO,
                                            ...
                                            ack, (uint16_t)sizeof(ack), &ack_length) == BOOTLOADER_RESULT_SUCCESS_E,
    ...
    check_result(bootloader_protocol_handle(&protocol, BOOTLOADER_PROTOCOL_CMD_DATA,
                         ...
                         BOOTLOADER_RESULT_PROTOCOL_ERROR_E &&
                     ack[0] == 0u,
...
static void test_metadata(void)
{
    ...
    check_result(bootloader_metadata_select(meta_a, meta_b, &output, &source) ==
                         BOOTLOADER_RESULT_SUCCESS_E &&
                     source == BOOTLOADER_FLASH_ZONE_META_B_E &&
                     output.copy_offset == 900u,
                 "newest redundant metadata copy is selected");
    meta_b[20] ^= 1u;
    check_result(bootloader_metadata_select(meta_a, meta_b, &output, &source) ==
                         BOOTLOADER_RESULT_SUCCESS_E &&
                     source == BOOTLOADER_FLASH_ZONE_META_A_E,
